...
public class ManagedConnectionFactoryImpl implements ManagedConnectionFactory,
    ...
    public ManagedConnection matchManagedConnections(Set set, Subject subject,
        ...
        for (Object candidate : set) {
            ...
            if(managedConnection.hasConnection()){
                log.warn("matched skipped: +" + managedConnection);
                continue;
            }
            if(managedConnection.isSessionInTransaction()){
                 log.warn("matched invalid: " + managedConnection);
                 if(log.isDebugEnabled()){
                     try {
	                for(javax.transaction.xa.Xid xid : managedConnection.getXAResource().recover(javax.transaction.xa.XAResource.TMNOFLAGS)){
	  	            log.debug("xid :"  + xid);
		        }
                        for(javax.transaction.xa.Xid xid : managedConnection.getXAResource().recover(javax.transaction.xa.XAResource.TMSTARTRSCAN)){
                            log.debug("xid istart:"  + xid);
                         }
                         for(javax.transaction.xa.Xid xid : managedConnection.getXAResource().recover(javax.transaction.xa.XAResource.TMENDRSCAN)){
                            log.debug("xid end:"  + xid);
                         }
                     } catch(Exception e){
                         log.debug("try recover", e);
                     }
                 }
                 continue;
            }
