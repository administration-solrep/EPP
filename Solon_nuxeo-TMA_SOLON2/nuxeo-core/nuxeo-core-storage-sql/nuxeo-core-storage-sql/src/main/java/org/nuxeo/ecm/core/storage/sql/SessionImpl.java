...
public class SessionImpl implements Session, XAResource {
    ...
    public List<Fragment> loadHierarchyFragments(Set<Serializable> hierIds) throws StorageException {
    	List<RowId> hierRowIds = new ArrayList<RowId>(hierIds.size());
    	for (Serializable id : hierIds) {
            hierRowIds.add(new RowId(Model.HIER_TABLE_NAME, id));
        }
    	return context.getMulti(hierRowIds, false);
    }
    
    public Set<Serializable> loadAncestorsIds(List<Fragment> hierFragments) throws StorageException {
    	final Set<Serializable> ancestors = new HashSet<Serializable>();
    	
    	final Set<Serializable> hierIds = new HashSet<Serializable>();
    	for(Fragment fragment : hierFragments){
    		Serializable id = ((SimpleFragment) fragment).get(Model.HIER_PARENT_KEY);
    		if(id != null){
    			hierIds.add(id);
    		}
    	}
    	if(!hierIds.isEmpty()){
	    	final List<Fragment> upperhierFragments = loadHierarchyFragments(hierIds);
	    	ancestors.addAll(loadAncestorsIds(upperhierFragments));
	    	ancestors.addAll(hierIds);
    	}
    ...
    public List<Node> getNodesByIds(List<Serializable> ids, boolean prefetch,
    		Set<String> tableNameToLoad) throws StorageException {
    	return getNodesByIds(ids, prefetch, tableNameToLoad, true);    	
    }
    
    private List<Node> getNodesByIds(List<Serializable> ids, boolean prefetch,
    		Set<String> tableNameToLoad, boolean loadAncestors)
            throws StorageException {
        // get hier fragments
        ...
        if(loadAncestors){ // SPL : compute ids of ancestors of all documents and load it
        	// to avoid loading one by one at document construction
        	Set<Serializable> ancestorids = loadAncestorsIds(hierFragments);        	
        	getNodesByIds(new ArrayList<Serializable>(ancestorids), true, null, false);
        }
    ...
    protected void findPrefetchedFragments(SimpleFragment hierFragment,
            List<RowId> bulkRowIds, Set<Serializable> proxyIds, Set<String> tableNameToLoad)
            throws StorageException {
        ...
        // find prefetch table names
        Set<String> prefetchTableNames = model.getTypePrefetchedFragments(typeName);
                
        if (prefetchTableNames == null && (tableNameToLoad == null || tableNameToLoad.isEmpty())) {
            return; // unknown (obsolete) type
        }
        
        Set<String> tableNames = new HashSet<String>();
        if(prefetchTableNames != null){
        	tableNames.addAll(prefetchTableNames);
        }
        if(tableNameToLoad != null){
        	tableNames.addAll(tableNameToLoad);
        }
    ...
    @Override
    public List<Node> getNodesByIds(List<Serializable> ids, Set<String> tableNameToLoad)
            throws StorageException {
        checkLive();
        return getNodesByIds(ids, true, tableNameToLoad);
    }
