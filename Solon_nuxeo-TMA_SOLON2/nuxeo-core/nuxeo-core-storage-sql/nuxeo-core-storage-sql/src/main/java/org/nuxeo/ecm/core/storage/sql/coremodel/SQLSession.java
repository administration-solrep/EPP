...
public class SQLSession implements Session {
    ...
    public List<Document> getDocumentsById(List<Serializable> ids, PrefetchInfo prefetchInfo)
        ...
        try {
        	Set<String> tableNameToLoad = null;
        	if(prefetchInfo != null){
        		tableNameToLoad = new HashSet<String>();
    	    	for (Field field : prefetchInfo.getFields()) {
    	            // prefetch all the relevant fragments
    	            Set<String> fragments = session.getModel().getFieldFragments(field);
    	            if (fragments != null) {
    	            	tableNameToLoad.addAll(fragments);
    	            }
    	        }
    	        for (Schema schema : prefetchInfo.getSchemas()) {
    	        	session.getModel().addSchemaFragments(schema, tableNameToLoad);
    	        }
        	}
        	
            List<Node> nodes = session.getNodesByIds(ids,  tableNameToLoad);
            for (Node node : nodes) {
                if (node == null) {
                    continue;
                }
