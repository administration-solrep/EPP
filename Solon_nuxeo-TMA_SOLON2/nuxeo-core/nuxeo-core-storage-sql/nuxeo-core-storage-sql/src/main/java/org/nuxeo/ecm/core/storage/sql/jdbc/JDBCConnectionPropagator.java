...
public class JDBCConnectionPropagator {
    ...
    public synchronized void addConnection(final JDBCConnection connection) {
        connections.add(connection);
    }

    public void setClusterNodeHandler(final ClusterNodeHandler clusterNodeHandler) {
        this.clusterNodeHandler = clusterNodeHandler;
    }

    public synchronized void removeConnection(final JDBCConnection connection) {
    ...
    */
    public synchronized void connectionWasReset(final JDBCConnection connection) throws StorageException {
        if (clusterNodeHandler != null && clusterNodeHandler.getConnection() == connection) {
            clusterNodeHandler.connectionWasReset();
        }
        for (final JDBCConnection c : connections) {
            if (c != connection) {
