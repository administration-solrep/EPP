...
public class VersionManagement {
    ...
    protected synchronized void load() {
        ...
        for (Node root : graph.getRoots()) {
            ...
            for (Dependency dep : (List<Dependency>)mgr.getDependencies()) {
                versions.put(makeKey(dep.getGroupId(), dep.getArtifactId(), dep.getType(), dep.getClassifier()), dep.getVersion());
            }
        }        
    }
