...
import static fr.dila.cm.caselink.CaseLinkConstants.CASE_REPOSITORY_NAME_FIELD;
...
import static fr.dila.cm.caselink.CaseLinkConstants.IS_SENT_FIELD;
...
import fr.dila.cm.core.service.GetMailboxesUnrestricted;
...
public class CreateCaseLink {
	 ...
	 *            The name of the repository in which the {@link CaseLink} will be created.
	 * @param subject
	 ...
	 * @param isSent
	 *            The post can be Sent or Received
	 * @param isInitial
	...
	public void create() throws ClientException {
		...
		recipient = mailboxes.get(0);
		DocumentModel doc = session.createDocumentModel(recipient.getDocument().getPathAsString(), UUID.randomUUID()
				.toString(), correspDocumentTypeService.getCaseLinkType());
