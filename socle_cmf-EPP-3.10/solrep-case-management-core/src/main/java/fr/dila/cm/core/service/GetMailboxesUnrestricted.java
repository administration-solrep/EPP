...
public class GetMailboxesUnrestricted extends UnrestrictedSessionRunner {
	...
	protected List<DocumentModel> getMailboxesDocumentModel (List<String> muids) throws ClientException {
		...
		try {
			...
			// On ne souhaite récupérer que le premier résultat et s'assurer qu'il n'y en a pas plusieurs donc 2 max
			qm.setMax(2);
			for (String muid : muids) {
				DocumentModelList res = qm.getDocuments(session, new Object[] { muid });
