...
public class MailboxManagementServiceImpl implements MailboxManagementService {
	...
	public Mailbox getMailbox(CoreSession session, String muid) {
		...
		DocumentModelList res = executeQueryModel(session, QUERY_GET_MAILBOX_FROM_ID, new Object[] { muid }, 2);
	...
	public List<Mailbox> getUserMailboxes(CoreSession session) {
		...
		DocumentModelList res = executeQueryModel(session, QUERY_GET_ALL_MAILBOX, new Object[] {}, 0);
	...
	@Override
	public List<Mailbox> createPersonalMailboxes(CoreSession session, DocumentModel userModel) {
		if (personalMailboxCreator == null) {
			throw new CaseManagementRuntimeException("Cannot create personal mailbox: missing creator configuration");
		}
		// First check if mailbox exists using unrestricted session to
		...
		if (hasMailbox(session, muid)) {
			log.error(String.format(
					...
					userModel.getName(), muid));
			return Arrays.asList(getMailbox(session, muid));
		}
		try {
			return personalMailboxCreator.createMailboxes(session, userModel);
		} catch (Exception e) {
			throw new CaseManagementRuntimeException(e.getMessage(), e);
		}
	}
	...
	public String getUserPersonalMailboxId(String user) {
		...
		return getUserPersonalMailboxId(userModel);
	}
