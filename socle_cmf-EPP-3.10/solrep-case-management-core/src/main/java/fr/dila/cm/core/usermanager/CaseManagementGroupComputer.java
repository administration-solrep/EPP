 ...
 *     Anahide Tchertchian
 */
package fr.dila.cm.core.usermanager;
...
public class CaseManagementGroupComputer extends AbstractGroupComputer {
	...
	public List<String> getGroupMembers(String groupName) throws Exception {
		GetMailboxInformationUnrestricted runner = new GetMailboxInformationUnrestricted(getRepoName(), getService(),
				groupName);
		...
		if (nuxeoPrincipal != null) {
			...
			if (!Boolean.TRUE.equals(disableRetrieveMailboxes.get())) {
				...
				try {
					...
					List<Mailbox> mailboxes = getService().getUserMailboxes(session);
