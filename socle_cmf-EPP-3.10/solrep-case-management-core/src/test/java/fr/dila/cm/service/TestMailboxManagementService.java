...
public class TestMailboxManagementService extends CaseManagementRepositoryTestCase {
	...
	public void testSearchMailboxes () throws Exception {
		...
		correspMailboxService.getUserMailboxes(session);
		correspMailboxService.getUserMailboxes(session);
		correspMailboxService.getUserMailboxes(session);
		correspMailboxService.getUserMailboxes(session);
	...
	public void testSendCase () throws Exception {
		...
		assertTrue(distributionService.getDraftCaseLink(session, senderMailbox, envelope.getDocument().getId())
				.isDraft());
		...
		DocumentModelList docList = session
				.query("select * from Document where ecm:mixinType = 'CaseLink' AND ecm:parentId = '"
						+ receiver1MailboxId + "'");
