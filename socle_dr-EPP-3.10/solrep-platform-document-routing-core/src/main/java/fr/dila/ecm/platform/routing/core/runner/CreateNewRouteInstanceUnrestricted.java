 ...
 */
package fr.dila.ecm.platform.routing.core.runner;
...
public class CreateNewRouteInstanceUnrestricted extends UnrestrictedSessionRunner {
	 ...
	 * @param session
	 * @param model
	 *            model of route. This document is not manipulated (ie: it is ok to use a document model)
	 * @param docIds
	 */
	public CreateNewRouteInstanceUnrestricted(CoreSession session, DocumentRoute model, List<String> docIds,
			boolean startInstance, DocumentRoutingPersister persister) {
		super(session);
		this.model = model;
		this.docIds = docIds;
	...
	public void run() throws ClientException {
		instance = persister.createDocumentRouteInstanceFromDocumentRouteModel(model.getDocument(), session);
		...
		fireEvent(session, routeInstance, props, DocumentRoutingConstants.Events.beforeRouteReady.name());
		routeInstance.setReady(session);
		fireEvent(session, routeInstance, props, DocumentRoutingConstants.Events.afterRouteReady.name());
		routeInstance.save(session);
		if (Framework.isTestModeSet()) {
			Framework.getLocalService(EventService.class).waitForAsyncCompletion();
		}
		if (startInstance) {
			fireEvent(session, routeInstance, null, DocumentRoutingConstants.Events.beforeRouteStart.name());
	...
	protected void fireEvent(CoreSession coreSession, DocumentRouteElement element,
