...
public class SessionImpl implements Session, XAResource {
    ...
    protected void markIndexingInProgress(Set<Serializable> dirtyIds) {
        ...
        for (Node node : getNodesByIds(dirtyIds, null)) {
    ...
    protected Node getNodeById(Serializable id, boolean prefetch) {
        List<Node> nodes = getNodesByIds(Collections.singletonList(id), prefetch, null);
    ...
    public List<Node> getNodesByIds(Collection<Serializable> ids, boolean prefetch, Set<String> tablesToLoad) {
        // get hier fragments
        ...
        if (prefetch) {
            ...
            for (Fragment fragment : hierFragments) {
                findPrefetchedFragments((SimpleFragment) fragment, bulkRowIds, proxyIds, tablesToLoad);
            }
            ...
            for (Fragment fragment : hierFragments) {
                findPrefetchedFragments((SimpleFragment) fragment, bulkRowIds, null, tablesToLoad);
            }
    ...
    protected void findPrefetchedFragments(SimpleFragment hierFragment, List<RowId> bulkRowIds,
            Set<Serializable> proxyIds, Set<String> tablesToLoad) {
        ...
        if (tableNames == null && (tablesToLoad == null || tablesToLoad.isEmpty())) {
            return; // unknown (obsolete) type
        }
        if(tableNames == null) {
        	tableNames = tablesToLoad;
        } else if(tablesToLoad != null) {
        	tableNames.addAll(tablesToLoad);
        }
    ...
    @Override
    public List<Node> getNodesByIds(Collection<Serializable> ids, Set<String> tablenamesToLoad) {
        checkLive();
        return getNodesByIds(ids, true, tablenamesToLoad);
    }
