...
public class NXQLQueryMaker implements QueryMaker {
        ...
        for (DocKind docKind : docKinds) {
            ...
            if (queryFilter.getPrincipals() != null) {
                ...
                if (dialect.supportsReadAcl()) {
                    ...
                    if (isSwordAclOptimEnabled() && dialect instanceof DialectOracle) {
                        securityClause = getReadAclsCheckSqlForOracle(racl + '.' + Model.HIER_READ_ACL_ACL_ID, (String[]) principals);
                        securityParams.addAll(Arrays.asList((String[]) principals));
                        securityParams.addAll(Arrays.asList((String[]) principals)); // add again for negative acls
                    } else {
                        securityClause = dialect.getReadAclsCheckSql(racl + '.' + Model.HIER_READ_ACL_ACL_ID);
    ...
    private boolean isSwordAclOptimEnabled() {
        return BooleanUtils.toBoolean(Framework.getProperty("sword.acl.optimization.enabled"));
    }

    private String getReadAclsCheckSqlForOracle(String idColumnName, String[] principals) {
        String questionMarks = String.join(",", Collections.nCopies(principals.length, "?"));
		String queryAclsFmt = "EXISTS (select /*+ NL_SJ */ a.acl_id FROM %s a WHERE a.acl_id = %s AND a.USERGROUP IN (%s))";
		String queryPositiveAcls = String.format(queryAclsFmt, "SW_ACLR_USER_ACLID", idColumnName, questionMarks);
		String queryNegativeAcls = String.format(queryAclsFmt, "SW_ACLR_DUSER_ACLID", idColumnName, questionMarks);
