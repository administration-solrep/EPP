    ...
    protected Principal doAuthenticate(CachableUserIdentificationInfo cachableUserIdent,
        ...
        try {
			...
			try {
				...
				if (bui != null) {
					if (!checkAndPopulateBruteForceDateInfos(bui, cachableUserIdent, httpRequest)) {
			...
			try {
				...
				checkAndPopulateBruteForceDateInfos(bui, cachableUserIdent, httpRequest);
			} catch (Exception e1) {
...
//            logAuthenticationAttempt(cachableUserIdent.getUserInfo(), false);
	 ...
	 */
	private boolean checkAndPopulateBruteForceDateInfos(BruteforceUserInfo bui,
			CachableUserIdentificationInfo cachableUserIdent, HttpServletRequest httpRequest) {
		Calendar canTryDate = bui.getLastAttemptDate();
		if(canTryDate == null) {
			canTryDate = Calendar.getInstance();
		}
        ...
        try {
            if (principal == null) {
                ...
                if (cachableUserIdent != null && cachableUserIdent.getUserInfo() != null) {
                    ...
                    if (LOGOUT_PAGE.equals(requestedPage) || !performAdditionalVerifications(principal)) {
                ...
                if (cachableUserIdent == null || cachableUserIdent.getUserInfo() == null) {
                    ...
                    if (userIdent != null && userIdent.containsValidIdentity()) {
                        ...
                        if (principal != null && principal != DIRECTORY_ERROR_PRINCIPAL) {
                            if (!performAdditionalVerifications(principal)) {
                                buildForbiddenResponse(httpResponse);
                                return;
                            }
                            ...
                            if (cachableUserIdent.getUserInfo().getLoginParameters()
                            	...
                            	session.setAttribute(NXAuthConstants.LOGIN_ERROR_MESSAGE, cachableUserIdent
                            ...
                            if (plugin instanceof LoginResponseHandler) {
								...
								httpRequest.setAttribute(NXAuthConstants.LOGIN_ERROR, err);
    ...
    /**
     * Performs additional verifications on principal
     * <p>
     * Method to be implemented in higher level applications
     * 
     * @param principal NuxeoPrincipal
     * @return true if principal is allowed to continue
     */
	protected boolean performAdditionalVerifications(Principal principal) { // NOSONAR principal parameter to be used for verifications
		return true;
	}
	 ...
	 * Retourne le temps d'attente en seconde à partir d'un nombre de tentative. attemptCount <= 15 en théorie
	 * @param attemptCount numéro de tentative
	 * @return temps d'attente en secondes
	 */
