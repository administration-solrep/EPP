...
public class NuxeoCorsCsrfFilter implements Filter {
    ...
    protected boolean manageCSRFToken(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (csrfTokenSkipPaths.stream().anyMatch(path::matches)) {
        ...
        if (session == null || (token = (String) session.getAttribute(CSRF_TOKEN_ATTRIBUTE)) == null) {
            ...
            sendUnauthenticateResponse(response);
        ...
        if (!token.equals(requestToken)) {
            ...
            if(request.getUserPrincipal()!=null || retrieveIdentityFromCache(request)!=null) {
    ...
    protected void sendUnauthenticateResponse(HttpServletResponse response) throws IOException {
        //Si la session ou le token vide on redirige sur / pour recharger
