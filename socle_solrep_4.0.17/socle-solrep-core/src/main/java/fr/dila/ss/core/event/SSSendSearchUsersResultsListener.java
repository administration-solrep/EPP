...
public class SSSendSearchUsersResultsListener implements PostCommitEventListener {
    private static final STLogger LOGGER = STLogFactory.getLog(SSSendSearchUsersResultsListener.class);
    ...
    private void handleEvent(Event event) {
        ...
        SSPrincipal principal = (SSPrincipal) eventCtx.getPrincipal();
        String recipient = principal.getEmail();

        boolean isAdmin = PermissionHelper.isAdminFonctionnel(principal);
        boolean isAdminMinisteriel = PermissionHelper.isAdminMinisteriel(principal);
        Set<String> ministeres = principal.getMinistereIdSet();

        CoreSession session = eventCtx.getCoreSession();
        DataSource excelFile = SSExcelUtil.exportResultUserSearch(
            session,
            usersDocs,
            isAdmin,
            isAdminMinisteriel,
