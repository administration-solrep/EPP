 ...
 * 
 * optionaly can check before doing anything that the eventName as an exepected value.
 * 
 * @author SPL
 * 
 */
public abstract class AbstractBatchEventListener extends AbstractEventListener {
	...
	@Override
	protected final void doHandleEvent(Event event) throws ClientException {
		if (eventName == null) {
			eventName = event.getName();
		} else if (!eventName.equals(event.getName())) {
			log.warn(null, STLogEnumImpl.UNEXPECTED_EVT_TEC,
					String.format(EVENT_EXPECTED_STR, eventName, event.getName()));
			return;
		}
		log.info(null, STLogEnumImpl.INIT_BATCH_TEC);
		log.debug(null, STLogEnumImpl.INIT_BATCH_TEC,
			...
			try {
				...
				if (batchLoggerModelFinal != null) {
					...
					if (event.getContext().hasProperty(STEventConstant.BATCH_EVENT_PROPERTY_PARENT_ID)) {
						long parentId = (Long) event.getContext().getProperty(
								STEventConstant.BATCH_EVENT_PROPERTY_PARENT_ID);
						batchLoggerModelFinal.setParentId(parentId);
					}
					suiviBatchService.flushBatchLogger(batchLoggerModelFinal);
					log.debug(session, STLogEnumImpl.END_BATCH_TEC, String.format(ETAT_TRANSACTION_STR,
							TransactionHelper.isTransactionActive(), isTransactionAlive()));
				}
			} finally {
	...
	protected abstract void processEvent(final CoreSession session, final Event event) throws ClientException;
	...
	/**
	 * Commit de la transaction et restart. En cas d'echec de du commit, on affiche en warn ou debug les logs suivant le
	 * booleen
	 * 
	 * @param session
	 * @param displayLogs
	 ...
	 */
	private void sendNotificationMails(final CoreSession session) throws ClientException {
		...
		if (errorCount > 0 && notifService.sontActiveesNotifications(session)) {
			...
			STServiceLocator.getSTMailService().sendTemplateHtmlMailToUserList(session,
					notifService.getAllUsers(session), objet, texte, params);
		}
	}
	
	protected enum JoursSemaine {
		LUNDI(Calendar.MONDAY), MARDI(Calendar.TUESDAY), MERCREDI(Calendar.WEDNESDAY), JEUDI(
				Calendar.THURSDAY), VENDREDI(Calendar.FRIDAY), SAMEDI(Calendar.SATURDAY), DIMANCHE(Calendar.SUNDAY);

		public Integer calValue;
