...
public abstract class AbstractFilterEventListener<T> extends AbstractEventListener {
	...
	@Override
	protected void doHandleEvent(Event event) throws ClientException {
		...
		if (contextClazz != null && !contextClazz.isAssignableFrom(event.getContext().getClass())) {
			LOG.warn("listener called on incorrect context (" + event.getContext().getClass() + "instead of "
					+ contextClazz + ")");
			return;
		}
	...
	protected abstract void doHandleEvent(Event event, T context) throws ClientException;
