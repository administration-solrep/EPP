...
public abstract class AbstractDailyReminderChangePasswordListener extends AbstractBatchEventListener {
	...
	protected void processEvent(final CoreSession session, final Event event) throws ClientException {
		...
		try {
			suiviBatchService.createBatchResultFor(batchLoggerModel, "Nombre de méls envoyé(s)", nbMailSent, endTime
					- startTime);
		} catch (Exception exc) {
	 ...
	 */
	protected abstract Set<STUser> getUsersList(final CoreSession session) throws ClientException;
	 ...
	 * @throws ClientException
	 */
	protected abstract int getDelayForUser(final CoreSession session, final STUser user) throws ClientException;
