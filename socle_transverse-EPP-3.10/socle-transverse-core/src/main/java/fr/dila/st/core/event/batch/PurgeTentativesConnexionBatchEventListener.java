...
public class PurgeTentativesConnexionBatchEventListener extends AbstractBatchEventListener {
	...
	protected void processEvent(CoreSession session, Event event) throws ClientException {
		...
		getPersistenceProvider().run(true, new PersistenceProvider.RunVoid() {
			@Override
		...
		try {
			suiviBatchService.createBatchResultFor(batchLoggerModel, "Purge des tentatives de connexion", endTime-startTime);
		} catch (Exception e) {
