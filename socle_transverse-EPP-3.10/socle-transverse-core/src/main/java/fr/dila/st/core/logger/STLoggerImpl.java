...
public class STLoggerImpl implements STLogger {
	...
	public STLoggerImpl(Class<?> classLoggante) {
		logger = Logger.getLogger(classLoggante);
	}
	 ...
	 */
	private String getTextCode(STLogEnum codeEnum, String message) {
		if (codeEnum == null) {
			logErrorUse();
			return null;
		} else {
	...
	private void logErrorUse() {
		...
		cleanMDCParameter();
		MDC.put(CODE_MDC, getFormattedCode(codeEnum));
		log(getTextCode(codeEnum, null), null, ERROR_INT);
		cleanMDCParameter();
	}
	 ...
	 *            le niveau de log
	 */
	private void simpleLog(final CoreSession session, final STLogEnum codeEnum, final Object object,
			final String message, final Throwable exc, int typeLog) {
		String messageCode = getTextCode(codeEnum, message);
		if (messageCode != null) {
	...
	private void simpleLog(final CoreSession session, final STLogEnum codeEnum, final Object object,
			final Throwable exc, int typeLog) {
		simpleLog(session, codeEnum, object, null, exc, typeLog);
	}

	private void multipleLog(final CoreSession session, final STLogEnum codeEnum, final List<Object> objects,
			final Throwable exc, int typeLog) {
	...
	private void multipleLog(final CoreSession session, final STLogEnum codeEnum, final Object[] objects,
			final Throwable exc, int typeLog) {
