 ...
 * 
 */
@Operation(id = UpdateListOperation.ID, category = CaseConstants.CASE_MANAGEMENT_OPERATION_CATEGORY, label = "UpdateList", description = "Met à jour le contenu d'une donnée multivaluée")
public class UpdateListOperation {
	...
	public static final String		EMPTY_PARAM_EXC			= "Les paramètres property et mode ne doivent pas être vides, id ou path doit également être renseigné";

	public static final String		BOTH_ID_PATH_EXC		= "Les paramètres id et path sont tous les deux renseignés. Un seul est nécessaire.";
	...
	public static final String		INCOMPATIBLE_TYPE_EXC	= "Impossible d'appliquer l'opération pour ce type de propriété : ";
	...
	public static final String		EMPTY_DATATYPE_EXC		= "La propriété à modifier est vide (aucune valeurs dans le document). Informations de dataType requis pour la propriété : ";
	...
	private void updateDocument(DocumentModel docToUpdate, Property propertyToUpdate, String inputParameters)
			throws OperationException, ClientException, IllegalAccessException {
		Serializable propertyValue = propertyToUpdate.getValue();
	...
	private Object[] runArray(Object[] currentValues, String inputParameters) throws OperationException,
		...
		if (REMOVEALL_MODE.equalsIgnoreCase(mode)) {
			valuesUpdated = Arrays.copyOf(currentValues, 0);
		} else {
	 ...
	 */
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private Map runMap(Map propertyValue, String inputParameters) throws OperationException, ClientException,
		...
		if (REMOVEALL_MODE.equalsIgnoreCase(mode)) {
			...
			if (isMapWellKnown(currentValues, inputParameters)) {
					...
					if (input.containsKey(currentEntry.getKey())) {
						...
						if (REMOVE_MODE.equalsIgnoreCase(mode)) {
							...
							if (value.getClass().isArray()) {
								...
								Object[] arrayUpdated = Arrays.copyOf(currentArray, currentArray.length
										+ arrayToAdd.length);
				...
				checkDataType();
				if (ADD_MODE.equalsIgnoreCase(mode)) {
					valuesUpdated = (HashMap<String, Object>) parseDataType(dataType, inputParameters);
				}
			}
		}
		return valuesUpdated;
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private List runList(List currentValues, String inputParameters) throws OperationException, ClientException {
		Object parametersValues = null;
	...
	private Map<String, Object> extractMapFromString(Map mapToMimic, String mapStr) throws OperationException,
		...
		if (mapStr.startsWith(MAP_INIT_STR) && mapStr.endsWith(MAP_END_STR)) {
			...
			parameters = StringUtil.removeEnd(parameters, MAP_END_STR);
			...
			for (int i = 0; i < mapKeysValues.length; i++) {
				...
				if (entry[1].startsWith(MAP_INIT_STR)) {
					parametersMap.put(entry[0].trim(),
	...
	private List<Object> splitListInfos(String listInfos, Object objectToMimic) throws OperationException,
		...
		if (listInfos.startsWith(LIST_INIT_STR) && listInfos.endsWith(LIST_END_STR)) {
			...
			parameters = StringUtil.removeEnd(parameters, LIST_END_STR);
	 ...
	 * @throws OperationException
	 */
	private Object parseDataType(String dataTypeToParse, String dataToConvert) throws ClientException,
			OperationException {
		...
		if (dataTypeToParse.startsWith(LIST_INIT_STR)) {
			...
			dataType = StringUtil.removeEnd(dataType, LIST_END_STR);
			dataConvert = dataConvert.replaceFirst(LIST_INIT_STR_REGEXP, "");
			dataConvert = StringUtil.removeEnd(dataConvert, LIST_END_STR);
			...
			dataType = StringUtil.removeEnd(dataType, MAP_END_STR);
			dataConvert = dataConvert.replaceFirst(MAP_INIT_STR_REGEXP, "");
			dataConvert = StringUtil.removeEnd(dataConvert, MAP_END_STR);
			...
			if (dataType.contains(MAP_SEPARATOR)) {
				...
				for (int i = 0; i < keysValuesTypes.size(); i++) {
					...
					map.put(keyValueValueArray[0].trim(),
	...
	private boolean compareObjects(Object obj, Object dataToUpdate) {
		...
		if (!obj.equals(dataToUpdate) && !isCalendar) {
			...
			if (dataInArray.get(Calendar.MINUTE) != dataToRemove.get(Calendar.MINUTE)
	...
	private Object getObjectFromSimpleName(@SuppressWarnings("rawtypes") Class element, String dataToConvert)
			throws OperationException, ClientException {
		return getObjectFromSimpleName(element.getSimpleName(), dataToConvert);
	}

	private Object getObjectFromSimpleName(String elementName, String dataToConvert) throws ClientException,
			OperationException {
		if (String.class.getSimpleName().equalsIgnoreCase(elementName)) {
			...
			return DateUtil.parseWithHour(dataToConvert);
	...
	private void checkParameters() throws OperationException {
		boolean isEmptyId = StringUtil.isEmpty(id);
		boolean isEmptyPath = StringUtil.isEmpty(path);
		if ((isEmptyId && isEmptyPath) || StringUtil.isEmpty(property) || StringUtil.isEmpty(mode)) {
		...
		if (!REMOVE_MODE.equalsIgnoreCase(mode) && !REMOVEALL_MODE.equalsIgnoreCase(mode)
	...
	public static String getModeException(String modeParameter) {
		...
		excStr.append("Mode ").append(modeParameter).append(" non reconnu ; ").append(REMOVE_MODE).append(", ")
				.append(REMOVEALL_MODE).append(", ").append(ADD_MODE).append(" autorisés");
