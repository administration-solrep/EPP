...
public class EntiteNodeImpl extends OrganigrammeNodeImpl implements EntiteNode {
	...
	public EntiteNodeImpl(EntiteNode node) throws ClientException {
		super(node);
		...
		if (StringUtil.isNotBlank(node.getMembreGouvernement())) {
	...
	public String getMembreGouvernement() {
		if (!StringUtil.isBlank(prenomMembre) && !StringUtil.isBlank(nomMembre)) {
			return prenomMembre + " " + nomMembre;
		} else {
			if (StringUtil.isNotBlank(prenomMembre)) {
				return prenomMembre;
			} else if (StringUtil.isNotBlank(nomMembre)) {
				return nomMembre;
			}
		}
		return "";
