...
public abstract class AbstractSTProfilUtilisateurServiceImpl implements STProfilUtilisateurService {

	/**
	 * Serial version UID
	 */
	private static final long		serialVersionUID	= -896758544406057019L;

	private static final STLogger	LOGGER				= STLogFactory
																.getLog(AbstractSTProfilUtilisateurServiceImpl.class);

	private static final String		QUERY_WORKSPACE		= "SELECT * FROM "
																+ STProfilUtilisateurConstants.WORKSPACE_DOCUMENT_TYPE
																+ STQueryConstant.WHERE
																+ STSchemaConstant.ECM_NAME_XPATH + " = '%s' AND "
	...
	@Override
	public STProfilUtilisateur getProfilUtilisateur(CoreSession session, String username) throws ClientException {
		DocumentModel profilUtilisateurDoc = getProfilUtilisateurDoc(session, username);
		if (profilUtilisateurDoc != null) {
			return profilUtilisateurDoc.getAdapter(STProfilUtilisateur.class);
		}
		return null;
	}

	@Override
	public DocumentModel getProfilUtilisateurDoc(CoreSession session, String username) throws ClientException {
		DocumentModel profilUtilisateurDoc = null;
		try {
			profilUtilisateurDoc = getProfilUtilisateurDocFromWorkspace(session, getUserWorkspaceDoc(session, username));
	...
	@Override
	public STProfilUtilisateur getProfilUtilisateurForCurrentUser(CoreSession session) throws ClientException {
		DocumentModel profilUtilisateurDoc = null;
		try {
			profilUtilisateurDoc = getProfilUtilisateurDocFromWorkspace(session,
					getUserWorkspaceDocForCurrentUser(session));
		} catch (WorkspaceNotFoundException we) {
			LOGGER.debug(session, STLogEnumImpl.FAIL_GET_WORKSPACE_FONC, we.getMessage());
			LOGGER.debug(session, STLogEnumImpl.FAIL_GET_WORKSPACE_FONC, we);
		}
	...
	@Override
	public DocumentModel getOrCreateCurrentUserProfil(final CoreSession session) throws ClientException {
	...
	@Override
	public int getNumberDayBeforeOutdatedPassword(CoreSession session, STUser user) throws ClientException {
		...
		int dayBeforeReminder = Integer.parseInt(paramService.getParametreValue(session,
				STParametreConstant.DELAI_PREVENANCE_RENOUVELLEMENT_MOT_DE_PASSE)) + 1;
		...
		String maxTimeOutdate = paramService.getParametreValue(session,
		...
		if (dateToChangePassword != null) {
			...
			if (todayReference.compareTo(dateToChangePassword) >= 0 || DateUtils.isSameDay(todayReference, dateToChangePassword)) {
				return 0;
			}

			for (int i = 1; i <= dayBeforeReminder; i++) {
				todayReference.add(Calendar.DAY_OF_MONTH, 1);
				if (todayReference.get(Calendar.DAY_OF_MONTH) == dateToChangePassword.get(Calendar.DAY_OF_MONTH)
	...
	@Override
	public boolean isUserPasswordOutdated(CoreSession session, String username) throws ClientException {
		...
		if (profilUtilisateur != null) {
			...
			String maxTimeOutdate = paramService.getParametreValue(session,
					STParametreConstant.DELAI_RENOUVELLEMENT_MOTS_DE_PASSE);
			todayReference.add(Calendar.MONTH, -Integer.parseInt(maxTimeOutdate));
	...
	@Override
	public void changeDatePassword(final CoreSession session, final String username) throws ClientException {
		new UnrestrictedSessionRunner(session) {
			@Override
			public void run() throws ClientException {
				final DocumentModel profilUtilisateurDoc = getOrCreateUserProfilFromId(session, username);
				final STProfilUtilisateur profilUtilisateur = profilUtilisateurDoc
						.getAdapter(STProfilUtilisateur.class);
	...
	@Override
	public Set<STUser> getToRemindChangePasswordUserList(CoreSession session) throws ClientException {
		final STParametreService paramService = STServiceLocator.getSTParametreService();
		final Set<STUser> userToRemind = new HashSet<STUser>();

		final Calendar refInf = Calendar.getInstance();
		final Calendar refSup = Calendar.getInstance();
		String reminderDate = paramService.getParametreValue(session,
				STParametreConstant.DELAI_PREVENANCE_RENOUVELLEMENT_MOT_DE_PASSE);
		String maxTimeOutdate = paramService.getParametreValue(session,
				STParametreConstant.DELAI_RENOUVELLEMENT_MOTS_DE_PASSE);
		refSup.add(Calendar.MONTH, -Integer.parseInt(maxTimeOutdate));
		refInf.add(Calendar.MONTH, -Integer.parseInt(maxTimeOutdate));
		...
		final String query = getReminderMDPQuery(DateUtil.formatYYYYMMdd(refInf.getTime()),
				DateUtil.formatYYYYMMdd(refSup.getTime()));
		...
		for (DocumentModel dm : allProfil) {
			...
			if (userModel == null) {
				LOGGER.warn(session, STLogEnumImpl.FAIL_GET_USER_TEC, "Pas d'utilisateur trouvé pour l'identifiant : "
						+ workspaceTitle);
			} else {
	...
	@Override
	public boolean isUserDeleted(String userId) throws ClientException {
		DocumentModel doc = STServiceLocator.getUserManager().getUserModel(userId);
	 ...
	 * @throws ClientException
	 */
	protected abstract DocumentModel getProfilUtilisateurDocFromWorkspace(CoreSession session,
			DocumentModel userWorkspaceDoc) throws ClientException;
	...
	protected DocumentModel createDefaultUserProfil(final CoreSession session, final String parentPath)
			throws ClientException {
		DocumentModel userProfilDocument = session.createDocumentModel(parentPath,
	 ...
	 * @throws ClientException
	 */
	protected DocumentModel getUserWorkspaceDoc(CoreSession session, String username) throws ClientException {
		...
		if (dml.isEmpty()) {
			...
			LOGGER.warn(session, STLogEnumImpl.ANO_WORKSPACE_MULTI_TEC, "Pour l'utilisateur <" + username
					+ "> nom du workspace <" + workspaceName + ">");
			return null;
		} else {
			return dml.get(0);
		}
	...
	protected DocumentModel getUserWorkspaceDocForCurrentUser(CoreSession session) throws WorkspaceNotFoundException,
		...
		DocumentModel doc = userWorkspaceService.getCurrentUserPersonalWorkspace(session, null);
		if (doc == null) {
	 ...
	 * @throws ClientException
	 */
	protected DocumentModel initProfilUtilisateurFromUserWorkspace(CoreSession session, DocumentModel userWorkspaceDoc)
			throws ClientException {
		if (userWorkspaceDoc != null) {
			final DocumentModel profilUtilisateurDocument = createDefaultUserProfil(session,
