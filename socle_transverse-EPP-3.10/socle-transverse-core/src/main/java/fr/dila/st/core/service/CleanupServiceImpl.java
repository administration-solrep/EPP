...
public class CleanupServiceImpl implements CleanupService {
	...
	private static final String		QUERY_DELETED_FMT		= "SELECT d."
	 ...
	 */
	@Override
	public int removeDeletedDocument(CoreSession session, String documentType, long nbSec) throws ClientException {
	...
	private int removeTheNFirstDocument(CoreSession session, String documentType, int nbDocsToDelete)
		...
		for (String id : ids) {
			...
			try {
				...
				LOGGER.debug(session, STLogEnumImpl.DEL_DOC_TEC, docRef,
						"Failed to remove [" + id + "] : " + e.getMessage(), e);
			}
		}
		return ids.size();
	}
