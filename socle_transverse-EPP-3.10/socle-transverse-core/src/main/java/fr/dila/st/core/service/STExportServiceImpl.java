...
public class STExportServiceImpl implements STExportService {
	...
	@Override
	public boolean isCurrentlyExporting(CoreSession session, DocumentModel exportDocumentDoc) throws ClientException {
	...
	@Override
	public boolean flagInitExport(CoreSession session, DocumentModel exportDocumentDoc) throws ClientException {
		if (exportDocumentDoc == null) {
			return false;
		} else {
	...
	public String getExportHorodatageRequest(CoreSession session, DocumentModel exportDocumentDoc)
		...
		if (exportDocumentDoc == null) {
			...
			ExportDocument exportDocument = exportDocumentDoc.getAdapter(ExportDocument.class);
			return DateUtil.formatWithHour(exportDocument.getDateRequest().getTime());
		}
	}

	@Override
	public DocumentModelList getExportDocumentDocs(CoreSession session, DocumentRef parentRef) throws ClientException {
	...
	@Override
	public String getOrCreateExportDocumentRootPath(CoreSession session, String parentPath, String folderExportName,
			String folderExportType) throws ClientException {
		DocumentModel exportDocumentRoot = getOrCreateExportDocumentRootDoc(session, parentPath, folderExportName,
				folderExportType);
	...
	@Override
	public DocumentModel getOrCreateExportDocumentRootDoc(CoreSession session, String parentPath,
			String folderExportName, String folderExportType) throws ClientException {
	...
	@Override
	public void flagEndExport(CoreSession session, DocumentModel exportDocumentDoc) throws ClientException {
		if (exportDocumentDoc != null) {
			ExportDocument exportDocument = exportDocumentDoc.getAdapter(ExportDocument.class);
			if (exportDocument.isExporting()) {
	...
	@Override
	public int removeOldExport(CoreSession session, Calendar dateLimit, String typeExport) throws ClientException {
		String query = "SELECT e.ecm:uuid as id FROM " + typeExport
