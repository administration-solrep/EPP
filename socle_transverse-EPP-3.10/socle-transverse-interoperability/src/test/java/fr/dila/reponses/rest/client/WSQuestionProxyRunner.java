...
public class WSQuestionProxyRunner {
	...
	public static void setup() {
		try {
			proxyFactory = new WSProxyFactory(ENDPOINT, BASEPATH, USERNAME, PASSWORD, null);
			Assert.assertNotNull(proxyFactory);

			service = proxyFactory.getService(WSQuestion.class);
	...
	public void testChercherQuestionJetonForMan() {
		...
		try {
			...
			String password = "ws_agriculture";
			// --- construction de la requête
			ChercherQuestionsRequest request = new ChercherQuestionsRequest();

			// --- récuperation du proxy client
			WSProxyFactory factory = new WSProxyFactory(endpoint, BASEPATH, username, password, null);
			WSQuestion service = factory.getService(WSQuestion.class);
			ChercherQuestionsResponse response = service.chercherQuestions(request);
			Assert.assertNotNull(response);
		} catch (Exception e) {
			e.printStackTrace();
