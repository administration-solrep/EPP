...
#
# Usage : creer_branche_reponses [release_dir]
#
# Script servant à créer une branche de livraison du projet Réponses.
#
# Argument:
...
export app_version="${REPONSES_VERSION}"
...
for i in ${projects} ; do ./creer_branche.sh "${i}" ; done
	...
	mkdir -p ${release_dir}/${app_name}/reponses/trunk/${app_distribution}/src/main/resources/
	cd ${release_dir}/${app_name}/reponses/trunk/${app_distribution}/src/main/resources/
	#on récupère le repertoire de distribution pour vider le dossier a_livrer dans un nouveau dossier avec numero de version
	svn co ${SVN_SERVER}/reponses/trunk/${app_distribution}/src/main/resources/init/
	cd init
	# Modification des repertoires "a_livrer";
		#chemin des repertoires update
	#variable prevention de passage de script à effectuer
	repLdapALivrer="${release_dir}/${app_name}/reponses/trunk/${app_distribution}/src/main/resources/init/ldap/update/a_livrer"
	repNxshellALivrer="${release_dir}/${app_name}/reponses/trunk/${app_distribution}/src/main/resources/init/nxshell/update/a_livrer"
	repOracleALivrer="${release_dir}/${app_name}/reponses/trunk/${app_distribution}/src/main/resources/init/oracle/update/a_livrer"
