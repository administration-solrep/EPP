function moveElement(element,containerId){
    ...
    Seam.Component.getInstance("dragAndDropActions").moveWithId(element.id,containerId,moveCallback);
}

function copyElement(element){
    //Seam.Remoting.contextPath = "/nuxeo";
    Seam.Remoting.getContext().setConversationId(currentConversationId);
    Seam.Component.getInstance("dragAndDropActions").copyWithId(element.id,copyCallback);
}
...
function pasteElement(element){
    //Seam.Remoting.contextPath = "/nuxeo";
    Seam.Remoting.getContext().setConversationId(currentConversationId);
    Seam.Component.getInstance("dragAndDropActions").pasteWithId(element.id,pasteCallback);
}
