...
public class ParamRefRelationCache<T> {
     ...
     */
    public DocumentRef retrieveDocumentRef(CoreSession session, T queryParamValue) {
        ...
        if (aref == null) {
            ...
            if (refs.length > 1) {
                throw new NuxeoException(
                    "Plus d'un document est retrourné pour [" + queryUFNXQL + "][" + queryParamValue + "]"
                );
     ...
     * @throws ClientException
     */
    public DocumentModel retrieveDocument(CoreSession session, T queryParamValue) {
