...
public class WidgetDescriptionDescriptor implements RequeteurWidgetDescription {
    ...
    public Boolean getHasToConvertLabel() {
        if (
            this.extraProperties.containsKey(
                    STRequeteurExpertConstants.REQUETEUR_PROPERTIES_DIRECTORY_HAS_TO_CONVERT_LABEL
                )
        ) {
            return this.getExtraProperties()
                .get(STRequeteurExpertConstants.REQUETEUR_PROPERTIES_DIRECTORY_HAS_TO_CONVERT_LABEL)
                .equals("true");
        } else {
    ...
    public String toString() {
        return String.format(
            "\t<widgetDescription category=\"%s\" name=\"%s\" type=\"%s\"/>",
            getCategory(),
            getName(),
            getType()
        );
