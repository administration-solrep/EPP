...
public class STAlertServiceImpl implements STAlertService, Serializable {
    ...
    private static final String NXQL_ACTIVE_ALERT =
        "SELECT * FROM " +
    ...
    @Override
    public Boolean sendMail(final CoreSession session, final Alert alert) {
            ...
            try {
                ...
                if (
                    requeteExperte
                        ...
                        .contains(STLifeCycleConstant.TO_DELETE_TRANSITION)
                ) {
                    session.followTransition(
                        requeteExperte.getDocument().getRef(),
