...
public class TestStSmartQuery {
    ...
    public void testDateValue() {
        ...
        String resultQuery = query.buildQuery();
        String dateStr = SolonDateConverter.DATE_DASH_REVERSE.format(today);
        // On ajoute un jour à la date pour l'inclusion
        Calendar tomorrow = Calendar.getInstance();
        tomorrow.setTime(today);
        tomorrow.add(Calendar.DAY_OF_MONTH, 1);
        String tomorrowStr = SolonDateConverter.DATE_DASH_REVERSE.format(tomorrow.getTime());

        Assert.assertEquals(
            "d.dos.numerorNor LIKE 'A*' AND d.dos:date1 BETWEEN DATE \"" +
            dateStr +
            "\" AND DATE \"" +
            tomorrowStr +
            "\"",
    ...
    public void testDynDateValue2() {
        ...
        tomorrow.setTime(today);
        tomorrow.add(Calendar.DAY_OF_MONTH, 1);
        String tomorrowStr = SolonDateConverter.DATE_DASH_REVERSE.format(tomorrow);

        String resultQuery = query.buildQuery();
        Assert.assertEquals(
            "d.dos.numerorNor LIKE 'A*' AND d.dos:date1 BETWEEN ufnxql_date:(NOW-10J) AND DATE \"" + tomorrowStr + "\"",
            resultQuery
        );
    }
}
