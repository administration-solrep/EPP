...
public class TestJetonService {
    @Inject
    private CoreFeature coreFeature;

    private String typeWebservice = "typeTest";
    ...
    @Test
    public void testAddDocumentInBasket() {
        try (CloseableCoreSession session = coreFeature.openCoreSession()) {
    ...
    @Test
    public void testGetNumeroJetonMaxForWs() {
        try (CloseableCoreSession session = coreFeature.openCoreSession()) {
            DocumentModelList dml = null;

            // remove any jeton
            dml = session.query("SELECT * from JetonDoc");
            dml.forEach(doc -> session.removeDocument(doc.getRef()));
            dml = session.query("SELECT * from JetonDoc");
            Assert.assertEquals(0, dml.size());
            ...
            Assert.assertEquals(-100, numeroJetonMax.longValue());
            ...
            Assert.assertEquals(-999, numeroJetonMax.longValue());
        }
    }
    ...
    public void testGetDocuments() {
        try (CloseableCoreSession session = coreFeature.openCoreSession()) {
            ...
            DocumentModel docJeton0 = createDocument(session, "/", "fichierTest", "File");
            jetonService.addDocumentInBasket(
                session,
                typeWebservice,
                owner,
                docJeton0,
                "numero dossier test 0",
                null,
                null
            );
            ...
            Assert.assertEquals(docJeton0.getId(), jetonDto.getDocumentList().get(0).getId());
            ...
            DocumentModel docJeton1_1 = createDocument(session, "/", "fichierTest2", "File");
            ...
            DocumentModel docJeton1_2 = createDocument(session, "/", "fichierTest3", "File");
            DocumentModel docJeton1_3 = createDocument(session, "/", "fichierTest4", "File");
            ...
            Assert.assertEquals(docJeton1_1.getId(), jetonDto.getDocumentList().get(0).getId());
            Assert.assertEquals(docJeton1_3.getId(), jetonDto.getDocumentList().get(1).getId());
            ...
            Assert.assertEquals(docJeton1_3.getId(), jetonDto.getDocumentList().get(0).getId());
    ...
    private void checkJetonDto(
        JetonServiceDto jetonDto,
        int nbDocumentsExpected,
        long nextJetonNumberExpected,
        boolean isLastSendingExpected
    ) {
        Assert.assertEquals(nbDocumentsExpected, jetonDto.getDocumentList().size());
        Assert.assertEquals(nextJetonNumberExpected, (long) jetonDto.getNextJetonNumber());
        Assert.assertEquals(isLastSendingExpected, (boolean) jetonDto.isLastSending());
    }

    private DocumentModel createDocument(CoreSession session, String pathParent, String nameDoc, String typeDoc) {
