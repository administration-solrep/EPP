...
public class WSProxyFactory {
	 ...
	 */
	public WSProxyFactory(String endpoint, String basePath, String username) {
		this(endpoint, basePath, username, null);
	}
	 ...
	 */
	public WSProxyFactory(String endpoint, String basePath, String username, String keyAlias) {
		super();
	 ...
	 * @return
	 */
	public final <T> T getService(Class<T> clazz, String password) throws WSProxyFactoryException {

		if (this.useStubs) {
			return getServiceStubs(clazz);
		} else {
			return getServiceProxy(clazz, password);
		}
	}
	...
	@SuppressWarnings("unchecked")
	private final <T> T getServiceProxy(Class<T> clazz, String password) throws WSProxyFactoryException {
		...
		if (clazz == WSAttribution.class) {
			proxy = new WSAttributionProxy(this.endpoint, this.basePath, this.username,password, this.keyAlias);
		} else if (clazz == WSControle.class) {
			proxy = new WSControleProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
		} else if (clazz == WSNotification.class) {
			proxy = new WSNotificationProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
		} else if (clazz == WSQuestion.class) {
			proxy = new WSQuestionProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
		} else if (clazz == WSReponse.class) {
			proxy = new WSReponseProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
		} else if (clazz == WSEpp.class) {
			proxy = new WSEppProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
		} else if (clazz == WSSpe.class) {
			proxy = new WSSpeProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
		} else if (clazz == WSEvenement.class) {
			proxy = new WSEvenementProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
			...
			proxy = new fr.dila.solonepp.rest.client.WSNotificationProxy(this.endpoint, this.basePath, this.username,
					password, this.keyAlias);
		} else if (clazz == WSEpg.class) {
			proxy = new WSEpgProxy(this.endpoint, this.basePath, this.username, password, this.keyAlias);
		} else {
			throw new WSProxyFactoryException("Can not create instance of " + clazz.getName()
					+ " : unsupported interface");
		}
