...
public class ActionAssembler {
    ...
    private static final ImmutableBiMap<Action, String> actionMap = new ImmutableBiMap.Builder<Action, String>()
            ...
            .put(Action.PASSAGE_MESSAGE_EN_COURS_DE_TRAITEMENT, SolonEppConstant.VERSION_ACTION_PASSER_MESSAGE_EN_COURS_DE_TRAITEMENT)
     ...
     */
    public static String assembleXsdToAction(Action xsd) throws ClientException {
        String action = actionMap.get(xsd);
        if (action == null) {
            throw new ClientException("Action inconnue : " + xsd);
        }
        return action;
    }
     ...
     * @throws ClientException
     */
    public static Action assembleActionToXsd(String action) throws ClientException {
        Action xsd = actionMap.inverse().get(action);
        if (xsd == null) {
            throw new ClientException("Action inconnue : " + action);
        }
        return xsd;
    }
}
