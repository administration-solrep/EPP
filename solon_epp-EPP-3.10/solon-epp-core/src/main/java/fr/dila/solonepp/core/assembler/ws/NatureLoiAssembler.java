...
/**
 * Assembleur des données nature loi vocabulaire <-> WS.
 * 
 * @author jtremeaux
 */
public class NatureLoiAssembler {
     ...
     */
    public static String assembleXsdToNatureLoi(NatureLoi wsId) throws ClientException {
        if (wsId == null) {
            return null;
        }
        final VocabularyService vocabularyService = STServiceLocator.getVocabularyService();
        String id = wsId.name();
        if (vocabularyService.hasDirectoryEntry(SolonEppVocabularyConstant.NATURE_LOI_VOCABULARY, id)) {
            return id;
        } else {
            throw new ClientException("Nature de loi inconnue : " + wsId);
        }
    }
     ...
     * @throws ClientException
     */
    public static NatureLoi assembleNatureLoiToXsd(String id) throws ClientException {
        if (StringUtils.isBlank(id)) {
            return null;
        }
        try {
            NatureLoi wsId = NatureLoi.valueOf(id);
            return wsId;
        } catch (IllegalArgumentException e) {
            throw new ClientException("Nature de loi inconnue : " + id);
        }
    }
}
