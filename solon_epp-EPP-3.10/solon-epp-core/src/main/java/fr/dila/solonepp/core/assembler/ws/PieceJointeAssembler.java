...
public class PieceJointeAssembler {
	...
	public static PieceJointe toPieceJointeXsd(DocumentModel pieceJointeDoc, boolean addPjContent)
		...
		fr.dila.solonepp.api.domain.evenement.PieceJointe pieceJointeAdapter = pieceJointeDoc
		...
		if (pieceJointeFichierDocList != null) {
			...
			for (DocumentModel pieceJointeFichierDoc : pieceJointeFichierDocList) {
				...
				contenuFichier.setNomFichier(pieceJointeFichier.getFilename());
				...
				if (addPjContent) {
					try {
						...
						throw new ClientException("Impossible de lire le contenu de la pièce jointe : "
								+ pieceJointeFichierDoc.getId(), e);
					}
	 ...
	 * @throws ClientException
	 */
	public static DocumentModel toPieceJointeDoc(CoreSession session, PieceJointe pieceJointe) throws ClientException {
		...
		fr.dila.solonepp.api.domain.evenement.PieceJointe adapted = pieceJointeDoc
				.getAdapter(fr.dila.solonepp.api.domain.evenement.PieceJointe.class);
		...
		for (ContenuFichier contenuFichier : contenusFichiers) {
			...
			if (fileName == null) {
				throw new ClientException("Le nom de la piece jointe est obligatoire.");
			}
			...
			if (contenuFichier.isADuContenu() != null && contenuFichier.isADuContenu()) {
				...
				if (contenu == null) {
					throw new ClientException("Contenu du fichier  '" + fileName + "' de pièce jointe vide.");
				}
				...
				if (contenuFichier.getSha512() != null && !digestSha512.equals(contenuFichier.getSha512())) {
					throw new ClientException("Le digest SHA512 du fichier '" + fileName
							+ "' est invalide. Digest attendu: " + digestSha512);
				}

				blob = FileUtils.createSerializableBlob(is, fileName, null);
	...
	public static PieceJointe toPieceJointeXsd(CoreSession session, DocumentModel pieceJointeDoc, String filename)
		...
		if (pjfList != null) {
			...
			for (DocumentModel pieceJointeFichierDoc : pjfList) {
				...
				if (pieceJointeFichier.getFilename().equals(filename)) {
					ContenuFichier contenuFichier = new ContenuFichier();
					contenuFichier.setNomFichier(pieceJointeFichier.getFilename());
					contenuFichier.setSha512(pieceJointeFichier.getDigestSha512());
					contenuFichier.setMimeType(pieceJointeFichier.getMimeType());
