 ...
 * 
 */
public class Evt19Assembler extends BaseAssembler {

    private final EppEvt19 eppEvt;

    public Evt19Assembler(final EppEvtContainer eppEvtContainer, final CoreSession session, final EppPrincipal principal) {
        super(session, principal);
        if (eppEvtContainer != null) {
            eppEvt = eppEvtContainer.getEvt19();
        } else {
            eppEvt = new EppEvt19();
        }
    }
    ...
    public void buildPieceJointe(final List<PieceJointe> pieceJointeList) {
        ...
        if (eppEvt.getLettrePmVersAn() != null) {
            pieceJointeList.add(eppEvt.getLettrePmVersAn());
        }
        if (eppEvt.getLettrePmVersSenat() != null) {
            pieceJointeList.add(eppEvt.getLettrePmVersSenat());
        }
    }

    @Override
    public EvenementType getEvenementType() {
        return EvenementType.EVT_19;
    }
    ...
    public void buildPieceJointeXsd(final List<DocumentModel> pieceJointeDocList, final boolean addPjContent) throws ClientException {
        ...
        if (pieceJointeDocList != null) {
            for (final DocumentModel pieceJointeDoc : pieceJointeDocList) {
                ...
                case LETTRE_PM_VERS_AN:
                    eppEvt.setLettrePmVersAn(pieceJointe);
                    break;
                case LETTRE_PM_VERS_SENAT:
                    eppEvt.setLettrePmVersSenat(pieceJointe);
                    break;
                }
            }
        }
    ...
    public void setEvtInContainer(final EppEvtContainer eppEvtContainerResponse) {
        eppEvtContainerResponse.setType(EvenementType.EVT_19);
        eppEvtContainerResponse.setEvt19(eppEvt);
    }
