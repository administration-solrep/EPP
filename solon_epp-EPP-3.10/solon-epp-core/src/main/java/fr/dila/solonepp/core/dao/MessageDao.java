...
public class MessageDao {
     ...
     */
    public MessageDao(CoreSession session, MessageCriteria criteria, PageInfo pageInfo) throws ClientException {
     ...
     * @throws ClientException
     */
    public MessageDao(CoreSession session, MessageCriteria criteria) throws ClientException {
    ...
    protected void build() throws ClientException {
        ...
        if (criteria.getOrderByList() != null && !criteria.getOrderByList().isEmpty()) {
            ...
            for (OrderByCriteria orderByCtrireia : criteria.getOrderByList()) {
                ...
                case ID_DOSSIER:
                    addOrderClause(orderBySb, orderByCtrireia.getOrder(), "m.", STSchemaConstant.CASE_LINK_SCHEMA_PREFIX,
                            SolonEppSchemaConstant.ID_DOSSIER);
                    joinEvenement = true;
                    break;
                case OBJET_DOSSIER:
                    addOrderClause(orderBySb, orderByCtrireia.getOrder(), "v.", SolonEppSchemaConstant.VERSION_SCHEMA_PREFIX,
                ...
                case HORODATAGE:
                    ...
                    break;
                case EMETTEUR:
                    addOrderClause(orderBySb, orderByCtrireia.getOrder(), "e.", SolonEppSchemaConstant.EVENEMENT_SCHEMA_PREFIX,
        ...
        if (joinDossier) {
            ...
            addJoinRestriction(criteriaList, "e.", STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_PARENT_ID_PROPERTY, "d.",
                    STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_UUID_PROPERTY);
        }

        if (joinEvenement) {
            addJoin(sb, SolonEppConstant.EVENEMENT_DOC_TYPE, "e");

            addJoinRestriction(criteriaList, "m.", STSchemaConstant.CASE_LINK_SCHEMA_PREFIX, STSchemaConstant.CASE_LINK_CASE_DOCUMENT_ID_PROPERTY,
                    "e.", STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_UUID_PROPERTY);
        }

        if (joinVersion) {
            addJoin(sb, SolonEppConstant.VERSION_DOC_TYPE, "v");

            addJoinRestriction(criteriaList, "m.", STSchemaConstant.CASE_LINK_SCHEMA_PREFIX,
                    SolonEppSchemaConstant.CASE_LINK_ACTIVE_VERSION_ID_PROPERTY, "v.", STSchemaConstant.ECM_SCHEMA_PREFIX,
        ...
        if (joinPieceJointe) {
            ...
            addJoinRestriction(criteriaList, "p.", STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_PARENT_ID_PROPERTY, "v.",
                    STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_UUID_PROPERTY);
        ...
        if (joinPieceJointeFichier) {
            ...
            addJoinRestriction(criteriaList, "p.", SolonEppSchemaConstant.PIECE_JOINTE_SCHEMA_PREFIX,
                    SolonEppSchemaConstant.PIECE_JOINTE_PIECE_JOINTE_FICHIER_LIST_PROPERTY, "f.", STSchemaConstant.ECM_SCHEMA_PREFIX,
                    STSchemaConstant.ECM_UUID_PROPERTY);
        ...
        if (criteria.getDossierAlerte() != null) {
            ...
            addDossierRestriction(SolonEppSchemaConstant.DOSSIER_ALERTE_COUNT_PROPERTY, comparator, criteriaList, value);

        }

        if (criteria.getCurrentLifeCycleState() != null) {
            addRestriction("m.", STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_CURRENT_LIFE_CYCLE_STATE, " = ? ", criteriaList,
                    criteria.getCurrentLifeCycleState());
        }

        if (StringUtils.isNotBlank(criteria.getCaseDocumentId())) {
            addMessageRestriction(STSchemaConstant.CASE_LINK_CASE_DOCUMENT_ID_PROPERTY, " = ? ", criteriaList, criteria.getCaseDocumentId());
        }

        if (StringUtils.isNotBlank(criteria.getEvenementId())) {
            String evenementId = criteria.getEvenementId().replaceAll("\\*", "%");

            addMessageRestriction(SolonEppSchemaConstant.ID_EVENEMENT, getEqualsOrLikeClause(evenementId), criteriaList, criteria.getEvenementId());

        }

        if (StringUtils.isNotBlank(criteria.getMessageType())) {
            addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_MESSAGE_TYPE_PROPERTY, " = ? ", criteriaList, criteria.getMessageType());
        }

        if (criteria.getMessageTypeIn() != null) {
            StringBuilder comparator = new StringBuilder(" IN (");
            comparator.append(StringUtil.getQuestionMark(criteria.getMessageTypeIn().size()));
            comparator.append(") ");

            addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_MESSAGE_TYPE_PROPERTY, comparator.toString(), criteriaList,
                    criteria.getMessageTypeIn());

        }

        if (StringUtils.isNotBlank(criteria.getCorbeille())) {
            addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_CORBEILLE_LIST_PROPERTY, " = ? ", criteriaList, criteria.getCorbeille());
        }

        if (criteria.getArNecessaire() != null) {
            addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_AR_NECESSAIRE_PROPERTY, " ? ", criteriaList, criteria.getArNecessaire() ? 1 : 0);
        }
        ...
        if (criteria.getArDonne() != null) {
            ...
            addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_AR_NON_DONNE_COUNT_PROPERTY, comparator, criteriaList, value);

        }

        // état de message
        if (StringUtils.isNotBlank(criteria.getEtatMessage())) {
            addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_ETAT_MESSAGE_PROPERTY, " = ? ", criteriaList, criteria.getEtatMessage());
        }
        ...
        if (criteria.getEtatMessageIn() != null && !criteria.getEtatMessageIn().isEmpty()) {
            ...
            StringBuilder comparator = new StringBuilder(" IN (");
            comparator.append(StringUtil.getQuestionMark(criteria.getEtatMessageIn().size()));
            comparator.append(") ");

            addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_ETAT_MESSAGE_PROPERTY, comparator.toString(), criteriaList,
        ...
        if (criteria.getEtatMessageExclus() != null && !criteria.getEtatMessageExclus().isEmpty()) {
            ...
            if (criteria.getDateTraitementMin() != null) {
                StringBuilder criteriaSb = new StringBuilder(" ((m.")
                        ...
                        // Exclusion d'état de message
                        .append(" AND m.").append(SolonEppSchemaConstant.CASE_LINK_SCHEMA_PREFIX).append(":")
                        .append(SolonEppSchemaConstant.CASE_LINK_ETAT_MESSAGE_PROPERTY).append(" IN (")
                        .append(StringUtil.getQuestionMark(criteria.getEtatMessageExclus().size()))
                        .append(")) ")

                        // Exclusion d'état de message
                        .append(" OR m.").append(SolonEppSchemaConstant.CASE_LINK_SCHEMA_PREFIX).append(":")
                        .append(SolonEppSchemaConstant.CASE_LINK_ETAT_MESSAGE_PROPERTY).append(" IN (")
                        .append(StringUtil.getQuestionMark(etatMessageForIn.size())).append(")) ");
                ...
                StringBuilder comparator = new StringBuilder(" IN (");
                comparator.append(StringUtil.getQuestionMark(etatMessageForIn.size()));
                comparator.append(") ");

                addMessageRestriction(SolonEppSchemaConstant.CASE_LINK_ETAT_MESSAGE_PROPERTY, comparator.toString(), criteriaList, etatMessageForIn);
        ...
        if (criteria.getCorbeilleMessageType() != null && criteria.getCorbeilleMessageType()) {
            StringBuilder criteriaSb = new StringBuilder(" ((m.")
                    ...
                    .append(" IN ('")
                    .append(SolonEppLifecycleConstant.MESSAGE_ETAT_WS_AR_RECU + "', '" + SolonEppLifecycleConstant.MESSAGE_ETAT_WS_EMIS + "', '"
                    ...
                    .append(SolonEppSchemaConstant.CASE_LINK_ETAT_MESSAGE_PROPERTY)
                    .append(" IN ('")
                    .append(SolonEppLifecycleConstant.MESSAGE_ETAT_WS_EN_ATTENTE_AR + "', '"
                            ...
                            + SolonEppLifecycleConstant.MESSAGE_ETAT_WS_NON_TRAITE).append("')) ");
            criteriaList.add(criteriaSb.toString());
        ...
        if (StringUtils.isNotBlank(criteria.getDossierId())) {
            ...
            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_DOSSIER_PROPERTY, getEqualsOrLikeClause(dossierId), criteriaList, dossierId);

        }

        if (StringUtils.isNotBlank(criteria.getEvenementType())) {
            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_TYPE_EVENEMENT_PROPERTY, " = ? ", criteriaList, criteria.getEvenementType());
        }

        if (criteria.getEvenementTypeIn() != null &&  criteria.getEvenementTypeIn().size() > 0) {

            StringBuilder comparator = new StringBuilder(" IN (");
            comparator.append(StringUtil.getQuestionMark(criteria.getEvenementTypeIn().size()));
            comparator.append(") ");

            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_TYPE_EVENEMENT_PROPERTY, comparator.toString(), criteriaList,
                    criteria.getEvenementTypeIn());
        }

        if (criteria.getEvenementCurrentLifeCycleState() != null) {
            addRestriction("e.", STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_CURRENT_LIFE_CYCLE_STATE, " = ? ", criteriaList,
                    criteria.getEvenementCurrentLifeCycleState());
        }

        if (StringUtils.isNotBlank(criteria.getEvenementEmetteur())) {
            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_EMETTEUR_PROPERTY, " = ? ", criteriaList, criteria.getEvenementEmetteur());
        }

        if (criteria.getEvenementEmetteurIn() != null) {

            StringBuilder comparator = new StringBuilder(" IN (");
            comparator.append(StringUtil.getQuestionMark(criteria.getEvenementEmetteurIn().size()));
            comparator.append(") ");

            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_EMETTEUR_PROPERTY, comparator.toString(), criteriaList,
                    criteria.getEvenementEmetteurIn());
        }

        if (StringUtils.isNotBlank(criteria.getEvenementDestinataire())) {
            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_DESTINATAIRE_PROPERTY, " = ? ", criteriaList,
        ...
        if (criteria.getEvenementDestinataireIn() != null) {
            ...
            StringBuilder comparator = new StringBuilder(" IN (");
            comparator.append(StringUtil.getQuestionMark(criteria.getEvenementDestinataireIn().size()));
            comparator.append(") ");

            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_DESTINATAIRE_PROPERTY, comparator.toString(), criteriaList,
                    criteria.getEvenementDestinataireIn());
        }

        if (StringUtils.isNotBlank(criteria.getEvenementDestinataireCopie())) {
            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_DESTINATAIRE_COPIE_PROPERTY, " = ? ", criteriaList,
        ...
        if (criteria.getEvenementDestinataireCopieIn() != null) {
            ...
            StringBuilder comparator = new StringBuilder(" IN (");
            comparator.append(StringUtil.getQuestionMark(criteria.getEvenementDestinataireCopieIn().size()));
            comparator.append(") ");

            addEvenementRestriction(SolonEppSchemaConstant.EVENEMENT_DESTINATAIRE_COPIE_PROPERTY, comparator.toString(), criteriaList,
                    criteria.getEvenementDestinataireCopieIn());
        }

        if (StringUtils.isNotBlank(criteria.getVersionSenat())) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_SENAT_PROPERTY, " = ? ", criteriaList, criteria.getVersionSenat());
        }

        if (StringUtils.isNotBlank(criteria.getVersionObjetLike())) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_OBJET_PROPERTY, " LIKE ? ", criteriaList, "%" + criteria.getVersionObjetLike() + "%");
        }

        if (criteria.getVersionHorodatageMin() != null) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_HORODATAGE_PROPERTY, " >= ? ", criteriaList, criteria.getVersionHorodatageMin());
        }

        if (criteria.getVersionHorodatageMax() != null) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_HORODATAGE_PROPERTY, " <= ? ", criteriaList, criteria.getVersionHorodatageMax());
        }

        if (criteria.getVersionNiveauLectureNumero() != null) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_NIVEAU_LECTURE_NUMERO_PROPERTY, " = ? ", criteriaList,
                    criteria.getVersionNiveauLectureNumero());
        }

        if (StringUtils.isNotBlank(criteria.getVersionNiveauLecture())) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_NIVEAU_LECTURE_PROPERTY, " = ? ", criteriaList, criteria.getVersionNiveauLecture());
        }

        if (criteria.getVersionPieceJointePresente() != null) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_PIECE_JOINTE_PRESENTE_PROPERTY, " = ? ", criteriaList,
                    criteria.getVersionPieceJointePresente() ? 1 : 0);
        }

        if (criteria.getVersionNumeroDepotTexte() != null) {
            addVersionRestriction(SolonEppSchemaConstant.VERSION_NUMERO_DEPOT_TEXTE_PROPERTY, " = ? ", criteriaList,
                    criteria.getVersionNumeroDepotTexte());
        }

        if (criteria.getDossierNumeroDepotTexte() != null) {
          addDossierRestriction(SolonEppSchemaConstant.DOSSIER_NUMERO_DEPOT_TEXTE_PROPERTY, " = ? ", criteriaList,
              criteria.getDossierNumeroDepotTexte());
        }

        if (criteria.getPieceJointeFichierFulltext() != null) {
            // TODO rapatrier QueryBuilder de naiad pour gerer es bind parameters sur l'attribut ecm:fulltext
            addRestriction("f.", STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_FULLTEXT_PROPERTY,
                    " = '" + criteria.getPieceJointeFichierFulltext() + "'", criteriaList, null);
        }
        ...
        if (criteria.getMetadonneeCriteria() != null) {
            ...
            for (Map.Entry<String, Object> entry : criteria.getMetadonneeCriteria().entrySet()) {
                ...
                if (value instanceof String && StringUtils.isNotBlank((String) value)) {
                    ...
                    if(!property.equals(SolonEppSchemaConstant.VERSION_NIVEAU_LECTURE_PROPERTY) &&
                    ...
                    if (!property.endsWith("_fin")) {
                        ...
                        addVersionRestriction(property, " >=? ", criteriaList, DateUtil.dateToGregorianCalendar((Date) value));

                    } else {

                        addVersionRestriction(property.replaceAll("_fin", ""), " <=? ", criteriaList, DateUtil.dateToGregorianCalendar((Date) value));
        ...
        if (criteria.isCheckReadPermission()) {
            ...
            addRestriction("m.", STSchemaConstant.ECM_SCHEMA_PREFIX, STSchemaConstant.ECM_PARENT_ID_PROPERTY, " = ? ", criteriaList,
                    mailboxDoc.getId());
        ...
        if (orderBySb != null) {
            ...
            sb.append(" ORDER BY ").append("v.").append(SolonEppSchemaConstant.VERSION_SCHEMA_PREFIX).append(":")
                    .append(SolonEppSchemaConstant.VERSION_HORODATAGE_PROPERTY).append(" DESC");
        }
    ...
    private boolean isJoinVersion(boolean joinPieceJointe) {
        return criteria.isJoinVersionForSorting() || joinPieceJointe || StringUtils.isNotBlank(criteria.getVersionSenat())
                || StringUtils.isNotBlank(criteria.getVersionObjetLike()) || criteria.getVersionHorodatageMin() != null
                || criteria.getVersionHorodatageMax() != null || criteria.getVersionNiveauLectureNumero() != null
                || StringUtils.isNotBlank(criteria.getVersionNiveauLecture()) || criteria.getVersionPieceJointePresente() != null
                || StringUtils.isNotBlank(criteria.getVersionNumeroDepotTexte()) || criteria.getMetadonneeCriteria() != null;
    }
    ...
    private boolean isJoinEvenement(boolean joinDossier) {
        return criteria.isJoinEvenementForSorting() || joinDossier || StringUtils.isNotBlank(criteria.getDossierId())
                || StringUtils.isNotBlank(criteria.getEvenementType()) || criteria.getEvenementTypeIn() != null
                || StringUtils.isNotBlank(criteria.getEvenementCurrentLifeCycleState()) || StringUtils.isNotBlank(criteria.getEvenementEmetteur())
                || StringUtils.isNotBlank(criteria.getEvenementDestinataire()) || StringUtils.isNotBlank(criteria.getEvenementDestinataireCopie())
                || criteria.getEvenementEmetteurIn() != null || criteria.getEvenementDestinataireIn() != null
    ...
    private void addRestriction(String alias, String prefix, String property, String comparator, List<String> criteriaList, Object value) {
    ...
    private void addJoinRestriction(List<String> criteriaList, String alias1, String prefix1, String property1, String alias2, String prefix2,
            String property2) {
    ...
    private List<String> getEtatMessageInclus(List<String> etatMessageExclus) {
        List<String> etatMessageInclus = new ArrayList<String>();
    ...
    public List<DocumentModel> list() throws ClientException {
        ...
        return QueryUtils.doUFNXQLQueryAndFetchForDocuments(session, SolonEppConstant.MESSAGE_DOC_TYPE, queryString, paramList.toArray(), pageSize,
                offset);
     ...
     * @throws ClientException
     */
    public DocumentModel getSingleResult() throws ClientException {
	...
	/**
	 * Recherche de messages par critères de recherche. Retourne plusieurs résultats.
	 * 
	 * @param criteria Critères de recherche
	 * @return List<DocumentModel> messages
	 * @throws ClientException
	 */
	public List<DocumentModel> getMultipleResults() throws ClientException {
		List<DocumentModel> list = list();
    ...
    private Calendar getDateTraitementMinFromParam() {
        ...
        try {
        	...
        	LOGGER.error(session, STLogEnumImpl.FAIL_GET_PARAM_TEC, SolonEppParametreConstant.NB_JOUR_MESSAGE_AFFICHABLE);
