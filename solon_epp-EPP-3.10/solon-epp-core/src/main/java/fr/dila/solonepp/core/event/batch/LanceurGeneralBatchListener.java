    ...
    protected void processEvent(CoreSession session, Event event) throws ClientException {
        ...
        try {
            ...
            for (DailyBatch dailyBatch : DailyBatch.values()) {
                InlineEventContext inlineEventContext = new InlineEventContext(session, session.getPrincipal(), eventProperties);
        ...
        try {
        	suiviBatchService.createBatchResultFor(batchLoggerModel, "Exécution du lanceur général", endTime-startTime);
        } catch (Exception e) {
