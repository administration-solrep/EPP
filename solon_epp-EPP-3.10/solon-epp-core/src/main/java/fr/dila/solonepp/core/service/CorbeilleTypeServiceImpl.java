...
public class CorbeilleTypeServiceImpl extends DefaultComponent implements CorbeilleTypeService {
	...
	private static final STLogger						LOGGER							= STLogFactory
																								.getLog(CorbeilleTypeServiceImpl.class);
	...
	@Override
	public List<CorbeilleNode> getCorbeilleInstitutionTree(String institution) throws ClientException {
		...
		if (corbeilleInstitutionDescriptor == null) {
			throw new ClientException(UNKNOWN_CORBEILLE + institution);
		}
	 ...
	 */
	private void addCorbeilleList(List<CorbeilleNode> corbeilleNodeList,
	 ...
	 */
	private void addCorbeilleList(List<CorbeilleNode> corbeilleNodeList,
			List<CorbeilleNodeDescriptor> corbeilleNodeDescriptorList, CoreSession session, Calendar dateDeTraitementMin) {
		if (corbeilleNodeDescriptorList == null) {
			return;
		}
		...
		for (CorbeilleNodeDescriptor corbeilleNodeDescriptor : corbeilleNodeDescriptorList) {
			...
			String query = "SELECT m.ecm:uuid as id FROM Message AS m , Evenement AS e , Version AS v  WHERE  m.cslk:caseDocumentId = e.ecm:uuid "
					+ "AND m.cslk:activeVersionId = v.ecm:uuid AND  m.cslk:corbeilleList = ? "
					+ "AND m.cslk:etatMessage IN ('EN_ATTENTE_AR', 'EN_COURS_REDACTION', 'NON_TRAITE') "
					+ "AND m.ecm:parentId = ?";
			...
			try {
				...
				if (mailboxDoc != null) {
					...
					count = QueryUtils.doCountQuery(session, QueryUtils.ufnxqlToFnxqlQuery(query), paramList.toArray())
							.toString();
				}
			...
			if (corbeilleNode.isTypeSection()) {
				List<CorbeilleNode> corbeilleSubNodeList = new ArrayList<CorbeilleNode>();
				addCorbeilleList(corbeilleSubNodeList, corbeilleNodeDescriptor.getCorbeilleNodeList(), session,
						dateDeTraitementMin);
				corbeilleNode.setCorbeilleNodeList(corbeilleSubNodeList);
			}
		}
	...
	public List<String> findCorbeilleDistribution(String institution, String messageType, String evenementType)
		...
		findCorbeilleDistribution(corbeilleList, corbeilleInstitutionDescriptor.getCorbeilleNodeList(), messageType,
				evenementType);
	 ...
	 * @throws ClientException
	 */
	protected void findCorbeilleDistribution(List<String> corbeilleList,
			List<CorbeilleNodeDescriptor> corbeilleNodeDescriptorList, String messageType, String evenementType)
			throws ClientException {
		for (CorbeilleNodeDescriptor corbeilleNodeDescriptor : corbeilleNodeDescriptorList) {
			if (SolonEppConstant.CORBEILLE_NODE_TYPE_SECTION.equals(corbeilleNodeDescriptor.getType())) {
				findCorbeilleDistribution(corbeilleList, corbeilleNodeDescriptor.getCorbeilleNodeList(), messageType,
						evenementType);
				...
				throw new ClientException("Type de corbeille inconnu : " + corbeilleNodeDescriptor.getType());
			}
		}
	}
	 ...
	 * @throws ClientException
	 */
	private boolean isMessageDistributedInCorbeilleType(CorbeilleNodeDescriptor corbeilleNodeDescriptor,
			String messageType, String evenementType) throws ClientException {
		for (MessageGroupDescriptor messageGroupDescriptor : corbeilleNodeDescriptor.getMessageGroupList()) {
			...
			if ("EMETTEUR".equals(messageType) && "EVT45".equals(evenementType) && evenementTypeSet.contains(evenementType)) {
				return true;
			}
		}
		return false;
