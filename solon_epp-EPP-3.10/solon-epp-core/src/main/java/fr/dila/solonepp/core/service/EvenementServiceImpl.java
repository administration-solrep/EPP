...
public class EvenementServiceImpl implements EvenementService {
    ...
    @Override
    public DocumentModel createBareEvenement(CoreSession session) throws ClientException {
    ...
    @Override
    public DocumentModel getEvenement(CoreSession session, String evenementId) throws ClientException {
        if (StringUtils.isBlank(evenementId)) {
            throw new ClientException("L'identifiant de la communication doit être renseigné");
        }
    ...
    @Override
    public List<DocumentModel> findEvenementSuccessif(CoreSession session, String evenementParentId) throws ClientException {
        EvenementCriteria evenementCriteria = new EvenementCriteria();
    ...
    @Override
    public DocumentModel creerEvenement(CoreSession session, DocumentModel evenementDoc, DocumentModel dossierDoc, boolean publie)
        ...
        if (evenement.getEmetteur() == null) {
        	if (principal.getInstitutionIdSet().size() > 1) {
        		throw new ClientException("Emetteur est obligatoire, mais non renseigné. On tente de le récupérer via les institutions de l'utilisateur, mais elles sont multiples. L'emetteur ne peut donc pas être déterminé.");
        	}
        ...
        if (StringUtils.isBlank(evenement.getTypeEvenement())) {
            throw new ClientException("Type de communication est obligatoire");
        }

        if (StringUtils.isBlank(evenement.getEmetteur())) {
            throw new ClientException("Emetteur est obligatoire");
        }
     ...
     // Valide les données de l'événement
        ...
        String sequenceId = String.format("%05d", sequencer.getNext("EVT_" + dossierTitle) - 1);
    ...
    @Override
    public DocumentModel modifierEvenementBrouillon(CoreSession session, DocumentModel currentEvenementDoc, DocumentModel newEvenementDoc)
            throws ClientException {
        ...
        evenementValidator.validateEtatEvenement(currentEvenementDoc,
                new HashSet<String>(Arrays.asList(new String[] { SolonEppLifecycleConstant.EVENEMENT_BROUILLON_STATE })));
    ...
    @Override
    public void publierEvenement(CoreSession session, DocumentModel evenementDoc) throws ClientException {
        
        LOGGER.info(session, STLogEnumImpl.PUBLICATION_COMMUNICATION_TEC, "communication : " + evenementDoc.getTitle() + ", UUID: " + evenementDoc.getId()) ;
        ...
        if (StringUtils.isNotBlank(evenementParentId)) {
            ...
            if (evtParentDoc != null) {
                ...
                if (!(evtParent.isEtatInstance() || evtParent.isEtatPublie() || evtParent.isEtatAttenteValidation())) {
                    throw new ClientException("La communication précédente doit être à l'état en instance ou publié");
                }
            }
        }
        ...
        if (evenement.isEtatInit()) {
            ...
            throw new ClientException("La communication doit être à l'état init ou brouillon pour être publié");
        }
    ...
    @Override
    public void annulerEvenement(CoreSession session, AnnulerEvenementContext annulerEvenementContext) throws ClientException {
        ...
        LOGGER.info(session, STLogEnumImpl.REQUEST_COMMUNICATION_CANCELLATION_TEC,"communication : " + annulerEvenementRequest.toString()) ;
        ...
        evenementValidator.validateEtatEvenement(
                evenementDoc,
                new HashSet<String>(Arrays.asList(new String[] { SolonEppLifecycleConstant.EVENEMENT_PUBLIE_STATE,
     ...
     * @param dossierDoc Dossier à annuler
     * @param evenementDoc Evenement à annuler
     * @return Document de la nouvelle version publiée pour annulation
     * @throws ClientException
     */
    protected DocumentModel annulerEvenementImmediatement(CoreSession session, DocumentModel dossierDoc, DocumentModel evenementDoc)
            throws ClientException {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);

        LOGGER.info(session, STLogEnumImpl.CANCEL_COMMUNICATION_TEC,"Annulation imediate de la communication : " + evenement.getTitle()) ;
        ...
        DocumentModel currentVersionDoc = versionService.getVersionActive(session, evenementDoc,
                SolonEppSchemaConstant.CASE_LINK_MESSAGE_TYPE_EMETTEUR_VALUE);
        if (currentVersionDoc == null) {
            throw new ClientException("Impossible de trouver la version active");
        }
        ...
        if (!currentVersion.isEtatPublie()) {
            throw new ClientException("La version active doit être à l'état publié");
        }
        ...
        final EvenementDistributionService evenementDistributionService = SolonEppServiceLocator.getEvenementDistributionService();
        evenementDistributionService.updateMessageAfterValider(session, dossierDoc, evenementDoc, versionCreatedDoc,
                SolonEppSchemaConstant.CASE_LINK_MESSAGE_TYPE_EMETTEUR_VALUE);
        evenementDistributionService.updateMessageAfterValider(session, dossierDoc, evenementDoc, versionCreatedDoc,
    ...
    /**
     * Annule un événement en créant une nouvelle version en attente de validation "pour annulation". L'événement doit être à l'état en instance.
     * 
     * @param session Session
     ...
     * @throws ClientException
     */
    protected DocumentModel annulerEvenementDemande(CoreSession session, DocumentModel dossierDoc, DocumentModel evenementDoc) throws ClientException {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);

        LOGGER.info(session, STLogEnumImpl.CANCEL_COMMUNICATION_TEC,"Annulation de la communication : " + evenement.getTitle() + ", création d'une nouvelle version pour annulation") ;
        ...
        final EvenementDistributionService evenementDistributionService = SolonEppServiceLocator.getEvenementDistributionService();
        evenementDistributionService.updateMessageAfterDemanderValidation(session, dossierDoc, evenementDoc, versionCreatedDoc);

        return versionCreatedDoc;
    }

    @Override
    public void supprimerEvenement(CoreSession session, DocumentModel evenementDoc) throws ClientException {

        LOGGER.info(session, STLogEnumImpl.DEL_COMMUNICATION_TEC,"communication : " + evenementDoc.getTitle() + ", UUID: " + evenementDoc.getId()) ;
    ...
    public void filterDestinataireCopie(DocumentModel evenementDoc) {
        ...
        Set<String> destinataireCopieExclusion = new HashSet<String>();
        destinataireCopieExclusion.add(evenement.getEmetteur());
        destinataireCopieExclusion.add(evenement.getDestinataire());
        List<String> filteredDestinataireCopie = new ArrayList<String>();
    ...
    @Override
    public void processEvenementAlerte(CoreSession session, DocumentModel evenementDoc, DocumentModel dossierDoc, DocumentModel versionDoc)
            throws ClientException {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        ...
        if (!evenementTypeService.isEvenementTypeAlerte(evenement.getTypeEvenement())) {
            ...
            if (positionAlerte) {
                ...
                if (evenementPoseAlerteDoc == null) {
                    throw new ClientException("Erreur technique : évenement de pose de l'alerte non trouvé");
                }

                // Lève l'alerte sur l'événement parent et tous les événements successifs : interdit de lever 2x l'alerte
     ...
     * @throws ClientException
     */
    protected void leveBrancheAlerte(CoreSession session, DocumentModel evenementDoc) throws ClientException {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        evenement.setBrancheAlerte(SolonEppSchemaConstant.EVENEMENT_BRANCHE_ALERTE_LEVEE_VALUE);
        session.saveDocument(evenementDoc);
     ...
     * 
     * @param session Session
     * @param evenementDoc Événement de départ.
     * @param filter Filtre de documents
     * @throws ClientException
     */
    protected DocumentModel getEvenementParent(CoreSession session, DocumentModel evenementDoc, Filter filter) throws ClientException {
    ...
    @Override
    public void initialiserEvenement(CoreSession session, InitialiserEvenementContext initialiserEvenementContext) throws ClientException {
        String idEvenementPrecendent = initialiserEvenementContext.getInitialiserEvenementRequest().getIdEvenementPrecedent();
        ...
        if (StringUtils.isEmpty(typeEvenement)) {
            throw new ClientException("Le type de communication ne peut etre vide");
        }
        if (StringUtils.isEmpty(idEvenementPrecendent) && !SolonEppServiceLocator.getEvenementTypeService().isTypeCreateur(typeEvenement)) {
            throw new ClientException("La communication précédente ne peut etre vide");
        }
        ...
        if (eventDocPrecedent == null && !SolonEppServiceLocator.getEvenementTypeService().isTypeCreateur(typeEvenement)) {
            throw new ClientException("Communication " + idEvenementPrecendent + " non trouve");
        }
        ...
        if (metaDonneesDescriptor.getEvenement() == null || metaDonneesDescriptor.getVersion() == null) {
            throw new ClientException("Metadonnées de la communication " + typeEvenement + " non trouvées");
        }
        ...
        Map<String, PropertyDescriptor> mapAllProperty = new HashMap<String, PropertyDescriptor>();
		...
		if (Lists.newArrayList(EvenementType.ALERTE, EvenementType.ALERTE_01, EvenementType.ALERTE_02,
				EvenementType.ALERTE_03, EvenementType.ALERTE_04, EvenementType.ALERTE_05, EvenementType.ALERTE_06,
				EvenementType.ALERTE_07, EvenementType.ALERTE_08, EvenementType.ALERTE_09, EvenementType.ALERTE_10,
				EvenementType.ALERTE_11, EvenementType.ALERTE_12, EvenementType.ALERTE_13, EvenementType.ALERTE_14,
				EvenementType.ALERTE_15).contains(EvenementType.fromValue(typeEvenement)) && eventDocPrecedent != null
        ...
        if (EvenementType.SD_02.equals(EvenementType.fromValue(typeEvenement))){
			...
			Evenement lastEvt = eventDocPrecedent != null ? eventDocPrecedent.getAdapter(Evenement.class) : null;
			if (lastVersionDoc != null && lastEvt != null
    ...
    private DocumentModel findEvenementByTitle(CoreSession session, String idEvenement) throws ClientException {
        EvenementCriteria evenementCriteria = new EvenementCriteria();
    ...
    @Override
    public List<DocumentModel> getEvenementsRacineDuDossier(CoreSession session, DocumentModel dossierDoc) throws ClientException {
        ...
        EvenementDao evenementDao = new EvenementDao(session);
        List<DocumentModel> evenementsDoc = evenementDao.getEvenementsRacine(evenementCriteria);

        return evenementsDoc;
    }

    @Override
    public List<DocumentModel> getEvenementDossierList(CoreSession session, DocumentModel dossierDoc) throws ClientException {
        ...
        evenementCriteria.setDossierId(dossier.getTitle());
        EvenementDao evenementDao = new EvenementDao(session);
        List<DocumentModel> evenementDocList = evenementDao.findEvenementByCriteria(evenementCriteria);

        return evenementDocList;
    }

    @Override
    public void envoyerMel(CoreSession session, String expediteurId, String objet, String corps, String destinataires, String copie,
            DocumentModel evenementDoc, DocumentModel versionDoc, DocumentModel dossierDoc, List<DocumentModel> pjDocList) throws ClientException {
        ...
        List<Address> destinairesAddress = new ArrayList<Address>();

        if (StringUtils.isBlank(expediteurId)) {
            throw new ClientException("Pas d'expéditeur");
        }
        
        if (StringUtils.isBlank(destinataires)) {
            throw new ClientException("Pas de destinataire");
        }

        if (StringUtils.isBlank(objet)) {
            throw new ClientException("Pas d'objet");
        }

        if (StringUtils.isBlank(corps)) {
            throw new ClientException("Pas de corps du message");
        }
        ...
        if (userDoc != null) {
            ...
            paramMap.put("expediteur", StringUtils.trimToEmpty(user.getEmail())
                    + " (" + StringUtils.trimToEmpty(user.getLastName()) + " "
                    + StringUtils.trimToEmpty(user.getFirstName()) + ")");
        }
        
        // Mail en copie
        List<Address> copiesAddress = new ArrayList<Address>();
        if (!StringUtils.isBlank(copie)) {
            try {
                copiesAddress.add(new InternetAddress(copie.trim()));
            } catch (AddressException e) {
                LOGGER.error(session, EppLogEnumImpl.FAIL_ADD_ADDRESS_TEC, "Erreur lors de l'ajout de l'adresse en copie : " + copie.trim()) ;
            }
        }

        paramMap.put("corps", corps);
        mailCorps = parametreService.getParametreValue(session, SolonEppParametreConstant.MAIL_TRANSMETTRE_EVENEMENT_CORPS);

        List<TemplatePropertyDTOImpl> metaDonneesList = new ArrayList<TemplatePropertyDTOImpl>();
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        MetaDonneesDescriptor metaDescriptor = metaDonneesService.getEvenementType(evenement.getTypeEvenement());
        ...
        for (String property : evtPropertyMap.keySet()) {
            ...
            Object value = null;
            if (propertyDescriptor.getName().equals(STSchemaConstant.DUBLINCORE_TITLE_PROPERTY)
        ...
        List<TemplatePropertyDTOImpl> pieceJointeDtoList = new ArrayList<TemplatePropertyDTOImpl>();
        List<ExportBlob> files = new ArrayList<ExportBlob>();
        ...
        if (pjDocList != null) {
            for (DocumentModel pjDoc : pjDocList) {
                ...
                for (DocumentModel pjfDoc : pjfDocList) {
                    ...
                    String valueStr = vocService.getEntryLabel(SolonEppVocabularyConstant.VOCABULARY_PIECE_JOINTE_DIRECTORY, pj.getTypePieceJointe());
                    templateProperty.setTitle(valueStr);
                    templateProperty.setValue(StringUtils.defaultString(pj.getNom()) + " - " + pjf.getFilename() + " - " + StringUtils.defaultString(pj.getUrl()));
        ...
        mailService.sendTemplateHtmlMailWithAttachedFiles(destinairesAddress, copiesAddress, objet, mailCorps, paramMap, files);
    }

    private TemplatePropertyDTOImpl getEvenementParam(CoreSession session, String property, Object value) throws ClientException {
        final VocabularyService vocService = STServiceLocator.getVocabularyService();
        ...
        templateDto.setTitle(I18NUtils.getMessageString(BUNDLE, "label.epp.version." + property, null, locale));
        ...
        if (property.equals("auteur") || property.equals("co_auteur") || property.equals("coauteur")) {
            if (value instanceof String) {
                ...
                List<String> identiteList = new ArrayList<String>();
        ...
        } else if (property.equals(SolonEppSchemaConstant.DOSSIER_COMMISSION_SAISIE_AU_FOND_PROPERTY) 
            ...
            if (value instanceof String) {
                ...
                if (organismeDoc != null) {
                    ...
                    LOGGER.warn(session, STLogEnumImpl.FAIL_GET_EVENT_PARAM_TEC,"Nom de l'organisme non trouvé - " + value) ;
                    valueStr = "";
                }
            } else if (value instanceof List) {
                @SuppressWarnings("unchecked")
                List<String> idList = (List<String>) value;
                List<String> organismeList = new ArrayList<String>();
            ...
            if(value instanceof String){
              valueStr = I18NUtils.getMessageString(BUNDLE, (String) value, null, locale);
            }else if(value instanceof List){
              @SuppressWarnings("unchecked")
            ...
            valueStr = DateUtil.formatDDMMYYYY((Calendar) value);
