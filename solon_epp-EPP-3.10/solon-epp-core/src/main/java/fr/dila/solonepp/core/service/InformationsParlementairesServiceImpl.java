...
public class InformationsParlementairesServiceImpl implements InformationsParlementairesService {
	...
	private static final String UNICITE_EVT45_QUERY = "SELECT v.ecm:uuid AS id FROM Version AS v, Evenement AS e WHERE v.ver:evenement != ? AND v.ver:evenement = e.evt:idEvenement AND v.ver:versionCourante = 1 AND v.ver:rubrique = ? AND v.ver:dateJo = ? AND e.evt:emetteur = ?";

	@Override
	public void callWsCreerDossierEpg(CoreSession session, Dossier dossier, Evenement evenement, Version version,
			boolean isPublier) throws Exception {
		...
		if (webservice != null) {
			...
			if (TraitementStatut.OK.equals(response.getStatut())) {
				...
				LOGGER.error(session, EppLogEnumImpl.FAIL_GET_WS_EPG_TEC, "Appel ws creer epg KO : " + response.getMessageErreur());
				throw new EppClientException("Retour KO de l'appel à CreerDossier");
			}
		} else {
			throw new EppClientException("Aucun poste webservice disponible pour l'appel à CreerDossier");
		}
		
	}

	@Override
	public void callWsModifierDossierEpg(CoreSession session, Evenement evenement, Version version,
			PieceJointe pieceJointe) throws Exception {
		LOGGER.debug(session, STLogEnumImpl.LOG_DEBUG_TEC, "###APPEL MODIFIER DOSSIER####");

		checkUnicite(session, version.getRubrique(), version.getDateJo(), evenement.getEmetteur(), evenement.getIdEvenement());
		...
		if (webservice != null) {
			...
			String rubriqueLabel = SolonEppServiceLocator.getSolonEppVocabularyService().getEntryLabel(SolonEppVocabularyConstant.RUBRIQUES_VOCABULARY, version.getRubrique());
			infos.setTitreActe("JO-01 : Insertion d'informations parlementaires au JOLD - " + rubriqueLabel + " - " + SDF_DD_MM_YYYY.format(Calendar.getInstance().getTime()));
			...
			//resp.setMailResponsable("ne-pas-repondre@dila.gouv.fr");
			...
			fichierParapheur.setNom(pieceJointeFichier.getFilename());
			...
			if (TraitementStatut.OK.equals(response.getStatut())) {
				...
				// statut ko on remonte l'erreur dans les logs
				LOGGER.error(session, EppLogEnumImpl.FAIL_GET_WS_EPG_TEC, "Appel ws modifier epg KO : " + response.getMessageErreur());
				throw new EppClientException("Retour KO de l'appel à ModifierDossier : " + response.getMessageErreur());
			}
		} else {
			throw new EppClientException("Aucun poste webservice disponible pour l'appel à ModifierDossier");
		}
	}
	
	private WSEpg getWsEpg() throws WSProxyFactoryException, ClientException {
		final List<PosteNode> posteNodeList = SolonEppServiceLocator.getOrganigrammeService().getPosteFromInstitution(InstitutionsEnum.GOUVERNEMENT.name());
		String url = null;
		...
		for (final PosteNode posteNode : posteNodeList) {
			...
			if (StringUtils.isNotBlank(url) && !posteNode.getDeleted() && posteNode.isActive() && url.contains("/site/solonepg")) {
		...
		WSProxyFactory factory = new WSProxyFactory(url, null, username, password, keyAlias);
		return factory.getService(WSEpg.class);
	 ...
	 */
	private void checkUnicite(final CoreSession session, final String rubrique, final Calendar dateJo, final String emetteur, final String idEvenement) throws ClientException {
		DocumentRef[] existingDoc = QueryUtils.doUFNXQLQueryForIds(session, UNICITE_EVT45_QUERY,
				new Object[] { idEvenement, rubrique, dateJo, emetteur }, 1, 0);
		if (existingDoc != null && existingDoc.length == 1) {
			...
			throw new EppClientException(String.format(
					...
					labelEmetteur, labelRubrique, SDF_DD_MM_YYYY.format(dateJo.getTime())));
		}
	}
	 ...
	 * @throws ClientException
	 */
	private void checkUniciteCreation(final CoreSession session, final String rubrique, final Calendar dateJo, final String emetteur) throws ClientException {
