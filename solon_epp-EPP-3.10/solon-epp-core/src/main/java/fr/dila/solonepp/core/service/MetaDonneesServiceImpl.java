...
public class MetaDonneesServiceImpl extends DefaultComponent implements MetaDonneesService {
	...
	@Override
	public DocumentModel remapDefaultMetaDonnees(DocumentModel docPrecedent, DocumentModel docSuivant, DocumentModel doc, Map<String, PropertyDescriptor> mapProperty)
	...
	@SuppressWarnings("unchecked")
	@Override
	public void remapConditionnelMetaDonnees(DocumentModel eventDoc, DocumentModel versionDoc, Map<String, PropertyDescriptor> mapProperty)throws ClientException {
		for (String name : mapProperty.keySet()) {
			DefaultValue defaultValue = mapProperty.get(name).getDefaultValue();
			if (defaultValue != null) {
				Object value = null;
