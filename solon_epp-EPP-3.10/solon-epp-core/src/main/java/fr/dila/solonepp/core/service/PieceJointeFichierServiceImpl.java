...
public class PieceJointeFichierServiceImpl implements PieceJointeFichierService {
	...
	@Override
	public DocumentModel getPieceJointeFichierRoot(final CoreSession session) throws ClientException {
		if (pieceJointeFichierRootDoc != null) {
			return pieceJointeFichierRootDoc;
		}
		synchronized (this) {
			final StringBuilder sb = new StringBuilder("SELECT pjr.ecm:uuid as id FROM ").append(
					SolonEppConstant.PIECE_JOINTE_FICHIER_ROOT_DOC_TYPE).append(" AS pjr");
			final String[] params = new String[] {};
			final List<DocumentModel> list = QueryUtils.doUnrestrictedUFNXQLQueryAndFetchForDocuments(session,
					SolonEppConstant.PIECE_JOINTE_FICHIER_ROOT_DOC_TYPE, sb.toString(), params, 1, 0);
			if (list == null || list.size() <= 0) {
				throw new ClientException("Racine des fichiers de pièces jointes non trouvée");
			}
	...
	@Override
	public DocumentModel createBarePieceJointeFichier(final CoreSession session) throws ClientException {
		return session.createDocumentModel(SolonEppConstant.PIECE_JOINTE_FICHIER_DOC_TYPE);
	}

	@Override
	public List<DocumentModel> updatePieceJointeFichierList(final CoreSession session,
			final DocumentModel evenementDoc, final DocumentModel pieceJointeDoc,
			final List<DocumentModel> pieceJointeFichierDocList, final boolean strictMode) throws ClientException {
		...
		final List<DocumentModel> currentPieceJointeFichierList = QueryUtils.retrieveDocuments(session,
				SolonEppConstant.PIECE_JOINTE_FICHIER_DOC_TYPE, currentPieceJointeFichierIdList);
		...
		for (final DocumentModel pieceJointeFichierDoc : pieceJointeFichierDocList) {
			...
			pieceJointeFichierFilenameSet.add(pieceJointeFichier.getFilename());
		}
		...
		for (final DocumentModel pieceJointeFichierDoc : currentPieceJointeFichierList) {
			final PieceJointeFichier pieceJointeFichier = pieceJointeFichierDoc.getAdapter(PieceJointeFichier.class);
			currentPieceJointeFichierMap.put(pieceJointeFichier.getFilename(), pieceJointeFichierDoc);
		}

		// Supprime les fichiers de pièce jointe qui ne sont plus dans la liste si non obligatoire
		for (final DocumentModel pieceJointeFichierDoc : currentPieceJointeFichierList) {
			final PieceJointeFichier pieceJointeFichier = pieceJointeFichierDoc.getAdapter(PieceJointeFichier.class);
			if (!pieceJointeFichierFilenameSet.contains(pieceJointeFichier.getFilename())) {
				if (!(pieceJointe.getModifiedFileList() != null && pieceJointe.getModifiedFileList().contains(
						pieceJointeFichier.getFilename())) && evenementTypeService.isPieceJointeObligatoire(typeEvenement, pieceJointe.getTypePieceJointe())) {
					if (strictMode) {
						throw new ClientException("Il est interdit de supprimer le fichier de pièce jointe obligatoire : "
								+ pieceJointeFichier.getFilename());
					}
				}
		...
		for (final DocumentModel pieceJointeFichierDoc : pieceJointeFichierDocList) {
			...
			final DocumentModel currentPieceJointeFichierDoc = currentPieceJointeFichierMap.get(pieceJointeFichier
					.getFilename());
			if (currentPieceJointeFichierDoc == null) {
				// Crée le nouveau fichier de pièce jointe
				final DocumentModel pieceJointeFichierCreatedDoc = createPieceJointeFichier(session, evenementDoc,
						pieceJointeFichierDoc);
				pieceJointeFichierAfterUpdateList.add(pieceJointeFichierCreatedDoc);
			} else {
				final PieceJointeFichier currentPieceJointeFichier = currentPieceJointeFichierDoc
				...
				if (!currentPieceJointeFichier.getDigestSha512().equals(pieceJointeFichier.getDigestSha512())) {
					if (!(pieceJointe.getModifiedFileList() != null && pieceJointe.getModifiedFileList().contains(
							pieceJointeFichier.getFilename())) && evenementTypeService.isPieceJointeObligatoire(typeEvenement, pieceJointe.getTypePieceJointe())) {
						if (strictMode) {
							throw new ClientException("Il est interdit de modifier le fichier de pièce jointe obligatoire : "
									+ pieceJointeFichier.getFilename());
						}
					}
	...
	@Override
	public DocumentModel createPieceJointeFichier(final CoreSession session, final DocumentModel evenementDoc,
			final DocumentModel pieceJointeFichierDoc) throws ClientException {
		final PieceJointeFichier pieceJointeFichier = pieceJointeFichierDoc.getAdapter(PieceJointeFichier.class);

		// Vérifie les données du fichier de pièce jointe
		if (StringUtils.isBlank(pieceJointeFichier.getFilename())) {
			throw new ClientException("Le nom du fichier de pièce jointe est obligatoire");
		}

		if (pieceJointeFichier.getContent() == null) {
			throw new ClientException("Le contenu du fichier de pièce jointe est obligatoire");
		}
		...
		LOGGER.info(session, STLogEnumImpl.CREATE_PIECE_JOINTE_TEC, "Pièce jointe: " + pieceJointeFichier.getFilename()
		...
		final String docTitle = evenementTitle + "_" + pieceJointeFichier.getFilename();
	...
	@Override
	public void removePieceJointeFichier(final CoreSession session, final DocumentModel pieceJointeDoc,
			final String pieceJointeFichierId) throws ClientException {
		...
		final List<DocumentModel> pieceJointeList = pieceJointeService.findPieceJointeByCriteria(session,
				pieceJointeCriteria);
	...
	@Override
	public List<DocumentModel> findAllPieceJointeFichier(final CoreSession session, final DocumentModel pieceJointeDoc)
