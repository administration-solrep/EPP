...
public class ProfilUtilisateurServiceImpl extends AbstractSTProfilUtilisateurServiceImpl implements ProfilUtilisateurService {
	 ...
	 * Serial version UID
	 */
	private static final long serialVersionUID = 1844382648125114381L;
    ...
    public List<STUser> filterUserForNotification(CoreSession session, final List<STUser> userList ) {
        ...
        sb.append(StringUtil.getQuestionMark(userList.size()));
	...
	@Override
	public List<STUser> getUserWithProfilList(CoreSession session) throws ClientException {
        ...
        for (STUser user : allUsersList) {
        	ProfilUtilisateur profilUtilisateur = getOrCreateCurrentUserProfil(session).getAdapter(ProfilUtilisateur.class);
	...
	protected String getReminderMDPQuery(String nxqlDateInf, String nxqlDateSup) {
		StringBuilder query = new StringBuilder("SELECT * FROM ").append(STProfilUtilisateurConstants.PROFIL_UTILISATEUR_DOCUMENT_TYPE)
			.append(" WHERE ").append("(pusr:dernierChangementMotDePasse >= DATE '").append(nxqlDateInf).append("')")
	...
	protected DocumentModel getProfilUtilisateurDocFromWorkspace(CoreSession session, DocumentModel userWorkspaceDoc) throws ClientException {
		if (userWorkspaceDoc != null) {
        	...
        	final PathRef profilUtilisateurRef = new PathRef(userWorkspacePath + "/" + STProfilUtilisateurConstants.PROFIL_UTILISATEUR_PATH);
