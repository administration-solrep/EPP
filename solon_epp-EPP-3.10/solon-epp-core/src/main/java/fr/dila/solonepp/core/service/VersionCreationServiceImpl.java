...
public class VersionCreationServiceImpl implements VersionCreationService {
	...
	@Override
	public void createVersion(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
			...
			if ("EVT45".equals(evenement.getTypeEvenement())) {
				...
				if (isPublier) {
					...
					try {
						...
						creerVersionContext.setPublie(false);
						LOGGER.error(session, EppLogEnumImpl.FAIL_GET_WS_EPG_TEC, "Échec de l'appel WS Modifier Dossier : création de la communication en brouillon", e);
					}
				}
			} else {
			...
			if ("EVT45".equals(evenement.getTypeEvenement())) {
				...
				if (creerVersionContext.isPublier()) {
					try {
						...
						LOGGER.error(session, EppLogEnumImpl.FAIL_GET_WS_EPG_TEC, "Échec de l'appel WS Modifier Dossier : pas de publication de la communication", e);
						throw new EppClientException(e);
					}
				}
			}
	...
	private void extraStepsForPublishEvt45(CoreSession session, CreerVersionRequest creerVersionRequest, Evenement evenement, Version version) throws Exception {
	...
	private void checkEvt45(Version version) throws ClientException {
		String rubrique = version.getRubrique();
		if (rubrique == null || rubrique.isEmpty()) {
			throw new EppClientException("La rubrique ne peut pas être vide pour ce type de communication.");
		}
		Calendar dateJo = version.getDateJo();
		if (dateJo == null) {
			throw new EppClientException("La date demandée de publication au JOLD ne peut pas être vide pour ce type de communication.");
		}
	}

	private void checkPiecesJointeEvt45(Evenement evenement, boolean etatBrouillon, List<DocumentModel> piecesJointeDocList) throws ClientException {
		if (piecesJointeDocList == null || piecesJointeDocList.isEmpty()) {
			throw new EppClientException("Les pieces jointes ne peuvent pas être vide pour ce type de communication.");
		}
	...
	private void checkChangementEmetteur(CoreSession session, Evenement modifiedEvenement) throws ClientException {
		...
		if (!currentEmetteur.equals(modifiedEmetteur)) {
			...
			if (!InstitutionsEnum.isInstitutionAlwaysAccessible(currentEmetteur)
	...
	private void setObjetFromRubrique(Version version) {
		...
		String rubrique = eppVocabularyService.getLabelFromId(SolonEppVocabularyConstant.RUBRIQUES_VOCABULARY, version.getRubrique(), STVocabularyConstants.COLUMN_LABEL);
		version.setObjet(rubrique);
	}

	@Override
	public void completerBrouillon(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        ...
        DocumentModel currentVersionDoc = versionService.getVersionActive(session, currentEvenementDoc, SolonEppSchemaConstant.CASE_LINK_MESSAGE_TYPE_EMETTEUR_VALUE);
        ...
        if (currentVersion.isEtatBrouillon()) {
            ...
            DocumentModel createdVersionDoc = versionService.modifierVersionBrouillonPourCompletion(session, currentVersionDoc, newVersionDoc, currentEvenementDoc);
            ...
            DocumentModel createdVersionDoc = versionService.creerVersionBrouillonPourCompletion(session, currentVersionDoc, newVersionDoc, currentEvenementDoc);
            creerVersionResponse.setVersionDoc(createdVersionDoc);
        } else {
            throw new ClientException("Impossible de modifier la version: " + currentVersion.getTitle());
        }
        ...
        List<DocumentModel> pieceJointeCreatedDocList = pieceJointeService.updatePieceJointeList(session, evenementDoc, versionCreatedDoc, pieceJointeToCreateDocList, true);
        creerVersionResponse.getPieceJointeDocList().addAll(pieceJointeCreatedDocList);
    }
    
    @Override
    public void completerPublier(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        CreerVersionResponse creerVersionResponse = creerVersionContext.getCreerVersionResponse();
        ...
        final DocumentModel versionCreatedDoc = versionService.publierVersionPourCompletion(session, currentVersionDoc, newVersionDoc, dossierDoc, currentEvenementDoc);
    ...
    @Override
    public void completerPublierDelta(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        ...
        if (versionContainer != null) {
            ...
            DocumentModel createdVersionBrouillonDoc = versionService.reporterVersionBrouillonPourCompletion(session, currentVersionBrouillonDoc, newVersionBrouillonDoc, currentEvenementDoc);
    
            // Crée, modifie ou supprime les pièces jointes
            List<DocumentModel> pieceJointeBrouillonToCreateDocList = versionContainer.getPieceJointeDocList();
            pieceJointeService.updatePieceJointeList(session, currentEvenementDoc, createdVersionBrouillonDoc, pieceJointeBrouillonToCreateDocList, true);
        }
    }

    @Override
    public CreerVersionResponse rectifierBrouillon(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        CreerVersionResponse creerVersionResponse = creerVersionContext.getCreerVersionResponse();
        ...
        if (currentVersion.isEtatBrouillon()) {
            ...
            DocumentModel createdVersionDoc = versionService.modifierVersionBrouillonPourRectification(session, currentVersionDoc, newVersionDoc, currentEvenementDoc);
            creerVersionResponse.setVersionDoc(createdVersionDoc);
        } else if (currentVersion.isEtatPublie() || currentVersion.isEtatAttenteValidation()) {
            // Crée une nouvelle version brouillon "pour rectification"
            DocumentModel createdVersionDoc = versionService.creerVersionBrouillonPourRectification(session, currentVersionDoc, newVersionDoc, currentEvenementDoc);
            creerVersionResponse.setVersionDoc(createdVersionDoc);
        } else {
            throw new ClientException("Impossible de modifier la version: " + currentVersion.getTitle());
        }
        ...
        final PieceJointeService pieceJointeService = SolonEppServiceLocator.getPieceJointeService();
        List<DocumentModel> pieceJointeToCreateDocList = creerVersionRequest.getPieceJointeDocList();
        List<DocumentModel> pieceJointeCreatedDocList = pieceJointeService.updatePieceJointeList(session, evenementDoc, versionCreatedDoc, pieceJointeToCreateDocList, false);
        creerVersionResponse.getPieceJointeDocList().addAll(pieceJointeCreatedDocList);
      return   creerVersionResponse;
    }
    
    @Override
    public CreerVersionResponse rectifierPublier(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        CreerVersionResponse creerVersionResponse = creerVersionContext.getCreerVersionResponse();
		...
		if ("EVT45".equals(evenement.getTypeEvenement())) {
			...
			if (currentVersion.isEtatBrouillon()) {
				...
				if (!SolonEppSchemaConstant.VERSION_MODE_CREATION_BROUILLON_RECTIFICATION_VALUE
						.equals(currentVersion.getModeCreation())) {
					throw new EppClientException("Seule la version brouillon pour rectification peut être modifiée");
				}
			} else if (!(currentVersion.isEtatPublie() || currentVersion.isEtatAttenteValidation())) {
				throw new EppClientException("Impossible de publier pour rectification la version: "
						+ currentVersion.getTitle() + " à l'état " + currentVersionDoc.getCurrentLifeCycleState());
			}
			setObjetFromRubrique(version);
			try {
				extraStepsForPublishEvt45(session, creerVersionRequest, evenement, version);
			} catch (Exception e) {
				LOGGER.error(session, EppLogEnumImpl.FAIL_GET_WS_EPG_TEC,
						"Échec de l'appel WS Modifier Dossier : pas de publication de la communication", e);
				throw new EppClientException(e);
			}
		}

        // Publie une nouvelle version pour rectification
        final DocumentModel versionCreatedDoc = versionService.publierVersionPourRectification(session, currentVersionDoc, newVersionDoc, dossierDoc, currentEvenementDoc, false);
        creerVersionResponse.setVersionDoc(versionCreatedDoc);
    ...
    @Override
    public void rectifierPublierDelta(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        CreerVersionResponse creerVersionResponse = creerVersionContext.getCreerVersionResponse();
        ...
        // Publie une nouvelle version pour rectification
        final VersionService versionService = SolonEppServiceLocator.getVersionService();
        final DocumentModel versionCreatedDoc = versionService.publierVersionPourRectification(session, currentVersionDoc, newVersionDoc, dossierDoc, currentEvenementDoc, true);
        creerVersionResponse.setVersionDoc(versionCreatedDoc);
        ...
        if (versionContainer != null) {
            ...
            // Reporte les modifications sur la version brouillon actuelle et renomme la version
            DocumentModel createdVersionBrouillonDoc = versionService.reporterVersionBrouillonPourRectification(session, currentVersionBrouillonDoc, newVersionBrouillonDoc, currentEvenementDoc);
     ...
     */
    protected void createEvenement(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        DocumentModel dossierDoc = creerVersionRequest.getDossierDoc();
        DocumentModel evenementDoc = creerVersionRequest.getEvenementDoc();
     ...
     * @throws ClientException
     */
    protected void createEvenementCreateur(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
     ...
     * @throws ClientException
     */
    protected void createEvenementSuccessif(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        DocumentModel dossierDoc = creerVersionRequest.getDossierDoc();
        DocumentModel evenementDoc = creerVersionRequest.getEvenementDoc();
        DocumentModel versionDoc = creerVersionRequest.getVersionDoc();
     ...
     * @throws ClientException
     */
    protected void createEvenementExistingDossier(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        CreerVersionResponse creerVersionResponse = creerVersionContext.getCreerVersionResponse();
        ...
        DocumentModel evenementCreatedDoc = evenementService.creerEvenement(session, evenementToCreateDoc, dossierDoc, publie);
     ...
     * @throws ClientException
     */
    protected void createVersionInitiale(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        CreerVersionResponse creerVersionResponse = creerVersionContext.getCreerVersionResponse();
        ...
        if (!creerVersionContext.isPublier()) {
            ...
            versionCreatedDoc = versionService.publierVersionInitiale(session, versionToCreateDoc, dossierDoc, evenementDoc);
        }
        creerVersionResponse.setVersionDoc(versionCreatedDoc);
     ...
     * @throws ClientException
     */
    protected void createVersionExistingEvenement(CoreSession session, CreerVersionContext creerVersionContext) throws ClientException {
        CreerVersionRequest creerVersionRequest = creerVersionContext.getCreerVersionRequest();
        CreerVersionResponse creerVersionResponse = creerVersionContext.getCreerVersionResponse();
        ...
        if (!newVersionPublie) {
            if (currentVersion.isEtatBrouillon()) {
                ...
                DocumentModel updatedVersionDoc = versionService.modifierVersionBrouillonInitiale(session, currentVersionDoc, newVersionDoc, currentEvenementDoc);
                ...
                if (currentEvenement.isEtatBrouillon()) {
                    ...
                    DocumentModel updatedEvenementDoc = evenementService.modifierEvenementBrouillon(session, currentEvenementDoc, modifiedEvenementDoc);
            ...
            DocumentModel pubishedVersionDoc = versionService.publierVersionBrouillonInitiale(session, currentVersionDoc, newVersionDoc, dossierDoc, currentEvenementDoc);
            creerVersionResponse.setVersionDoc(pubishedVersionDoc);
            
		} else {
			throw new ClientException("Interdiction de créer une nouvelle version à l'état publié de la communication "
					+ modifiedEvenementDoc.getAdapter(Evenement.class).getTitle());
		}
        ...
        final PieceJointeService pieceJointeService = SolonEppServiceLocator.getPieceJointeService();
        List<DocumentModel> pieceJointeToCreateDocList = creerVersionRequest.getPieceJointeDocList();
        List<DocumentModel> pieceJointeCreatedDocList = pieceJointeService.updatePieceJointeList(session, modifiedEvenementDoc, versionCreatedDoc, pieceJointeToCreateDocList, false);
        creerVersionResponse.getPieceJointeDocList().addAll(pieceJointeCreatedDocList);
    }
}
