...
public class VersionAssembler {
     ...
     */
    public void assembleVersionForUpdate(
        DocumentModel versionFromDoc,
        DocumentModel versionToDoc,
        ...
        for (PropertyDescriptor propertyDescriptor : versionMetaDonneesDescriptor.getProperty().values()) {
            ...
            if (SolonEppSchemaConstant.VERSION_SENAT_PROPERTY.equals(property) && !principal.isInstitutionSenat()) {
                continue;
            }
            ...
            if (propertyDescriptor.isMultiValue()) {
                if (initial || !propertyDescriptor.isObligatoire() || !completion) {
                    ...
                    versionToDoc.setProperty(
                        schema,
                        property,
                        assembleListValue(valueFrom, valueTo, schema, propertyDescriptor.getName(), false)
                    );
                } else {
                    ...
                    versionToDoc.setProperty(
                        schema,
                        property,
                        assembleListValue(valueFrom, valueTo, schema, propertyDescriptor.getName(), true)
                    );
                }
            } else {
                if (initial || !propertyDescriptor.isObligatoire() || !completion) {
                    ...
                    if (!equals) {
                        StringBuilder errorMsg = new StringBuilder("La propriété obligatoire ")
                            ...
                            .append(valueFrom);
                        throw new NuxeoException(errorMsg.toString());
                    }
                }
            }
        }
    ...
    public List assembleListValue(Object valueFrom, Object valueTo, String schema, String property, boolean addMode) {
        ...
        if (valueFrom == null) {
            ...
            if (!(valueTo instanceof Collection)) {
                throw new NuxeoException(
                    "La propriété " + schema + ":" + property + " des données entrantes doit contenir une collection"
                );
            }
        }
            ...
            if (valueTo != null) {
                if (!(valueTo instanceof Collection)) {
                    throw new NuxeoException(
                        ...
                        ":" +
                        property +
                        " du document modifié entrantes doit être une collection"
                    );
                }
        ...
        if (!addMode) {
            ...
            for (Object o : valueToSet) {
                if (valueFromSet.count(o) < valueToSet.count(o)) {
                    throw new NuxeoException(
                        "Il est interdit de retirer un élément de la propriété multivaluée " +
                        schema +
                        ":" +
                        property +
                        " valeur :" +
