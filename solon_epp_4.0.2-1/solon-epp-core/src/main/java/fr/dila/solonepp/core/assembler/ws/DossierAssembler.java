...
public class DossierAssembler {
     ...
     */
    public fr.sword.xsd.solon.epp.Dossier assembleDossierToXsd(CoreSession session, DocumentModel dossierDoc) {
        ...
        if (
            StringUtils.isNotBlank(dossier.getCommissionSaisieAuFond()) || dossier.getCommissionSaisiePourAvis() != null
        ) {
        ...
        if (dossier.getDateDistributionElectronique() != null) {
            ficheDossier.setDateDistribution(
                DateUtil.calendarToXMLGregorianCalendar(dossier.getDateDistributionElectronique())
            );
        }

        if (dossier.getDateEngagementProcedure() != null) {
            ficheDossier.setDateEngagementProcedure(
                DateUtil.calendarToXMLGregorianCalendar(dossier.getDateEngagementProcedure())
            );
        }
        ...
        if (dossier.getDateRefusProcedureEngagementAn() != null) {
            ficheDossier.setDateRefusAn(
                DateUtil.calendarToXMLGregorianCalendar(dossier.getDateRefusProcedureEngagementAn())
            );
        }

        if (dossier.getDateRefusProcedureEngagementSenat() != null) {
            ficheDossier.setDateRefusSenat(
                DateUtil.calendarToXMLGregorianCalendar(dossier.getDateRefusProcedureEngagementSenat())
            );
        }
     ...
     */
    public Commission assembleCommissionDossierToXsd(DocumentModel dossierDoc) {
        Dossier dossier = dossierDoc.getAdapter(Dossier.class);
        ...
        if (
            ...
            DocumentModel organismeDoc = tableReferenceService.getOrganismeById(
                session,
                dossier.getCommissionSaisieAuFond()
            );
            if (organismeDoc != null) {
     ...
     * @return Objet dépot WS
     */
    public Depot assembleDepotRapportDossierToXsd(DocumentModel dossierDoc) {
        Dossier dossier = dossierDoc.getAdapter(Dossier.class);
        Depot depot = null;
        if (dossier.getDateDepotRapport() != null || !StringUtils.isBlank(dossier.getNumeroDepotRapport())) {
            depot = new Depot();
     ...
     * @return Objet dépot WS
     */
    public Depot assembleDepotTexteDossierToXsd(DocumentModel dossierDoc) {
        Dossier dossier = dossierDoc.getAdapter(Dossier.class);
        Depot depot = null;
        if (dossier.getDateDepotTexte() != null || !StringUtils.isBlank(dossier.getNumeroDepotTexte())) {
            depot = new Depot();
     ...
     */
    public NiveauLecture assembleNiveauLectureDossierToXsd(DocumentModel dossierDoc) {
        Dossier dossier = dossierDoc.getAdapter(Dossier.class);
        ...
        if (dossier.getNiveauLecture() != null) {
            NiveauLectureCode niveauLectureCode = NiveauLectureCodeAssembler.assembleNiveauLectureCodeToXsd(
                dossier.getNiveauLecture()
            );
