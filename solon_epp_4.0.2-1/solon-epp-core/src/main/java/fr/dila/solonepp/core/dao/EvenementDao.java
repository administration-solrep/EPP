...
public class EvenementDao {
     ...
     */
    public List<DocumentModel> findEvenementByCriteria(final EvenementCriteria criteria) {
        final StringBuilder sb = new StringBuilder("SELECT e.ecm:uuid as id FROM ")
            .append(SolonEppConstant.EVENEMENT_DOC_TYPE)
            .append(" AS e ");
        ...
        final List<DocumentModel> list = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
            session,
            SolonEppConstant.EVENEMENT_DOC_TYPE,
            sb.toString(),
            paramList.toArray()
        );
     ...
     */
    public DocumentModel getSingleEvenementByCriteria(final EvenementCriteria criteria) {
     ...
     */
    public List<DocumentModel> getEvenementsRacine(final EvenementCriteria criteria) {
        if (criteria == null || StringUtils.isEmpty(criteria.getDossierId())) {
            throw new NuxeoException("dossierTitle ne doit pas être null");
        }
