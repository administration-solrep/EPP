...
public class VersionDao {
     ...
     */
    public List<DocumentModel> findVersionByCriteria(final VersionCriteria criteria) {
        ...
        long count = QueryHelper.doUfnxqlCountQuery(session, sb.toString(), paramList.toArray());
        if (!QueryHelper.checkCount(session, count, sb, criteria)) {
        ...
        final List<DocumentModel> list = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
            session,
            SolonEppConstant.VERSION_DOC_TYPE,
            sb.toString(),
            paramList.toArray(),
            pageSize,
            offset
    ...
    private List<Object> getQuery(final VersionCriteria criteria, final StringBuilder query) {
        ...
        if (criteria.getCurrentLifeCycleStateIn() != null) {
            ...
            sb.append(STSchemaConstant.ECM_CURRENT_LIFE_CYCLE_STATE);
            sb.append(" IN (");
            sb.append(StringHelper.getQuestionMark(criteria.getCurrentLifeCycleStateIn().size()));
            sb.append(") ");
        ...
        if (!criterialist.isEmpty()) {
            query.append(" WHERE testAcl(v.ecm:uuid) = 1 AND ").append(StringUtils.join(criterialist, " AND "));
        }
     ...
     */
    public DocumentModel getSingleVersionByCriteria(final VersionCriteria criteria) {
    ...
    public List<VersionSelectionDTO> findVersionSelectionnable(final VersionCriteria criteria) {
        final StringBuilder sb = new StringBuilder(
        ...
        try {
            ...
            for (final Map<String, Serializable> result : res) {
                listResult.add(
                    new VersionSelectionDTOImpl(
                        (String) result.get("id"),
                        (String) result.get("title"),
                        (String) result.get("description"),
                        (String) result.get("etat"),
                        (Calendar) result.get("dateAr")
                    )
                );
            }
        } finally {
