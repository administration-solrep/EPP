...
public class XsdToUfnxqlParser {
    ...
    public String parse(
        ...
        if (params == null) {
            throw new NuxeoException("La liste de paramètres ne peut pas être nulle.");
        }
        ...
        try {
            ...
            throw new NuxeoException(exc.getMessage());
        }
        ...
        if (StringUtils.isBlank(xsdQuery)) {
            throw new NuxeoException("La requête ne peut être vide.");
        }
        ...
        String clause = xSDQueryBuilder.getWhereClause().getClause();
        ...
        params.clear();
        params.addAll(xSDQueryBuilder.getWhereClause().getParams());
        ...
        if (quoteParameters) {
            queryParameter = StringUtils.wrap(queryParameter, '\'');
        }
        ...
        if (StringUtils.isNotBlank(orderClause)) {
            ...
            return selectClause.append(whereClause).append(" ORDER BY ").append(orderClause).toString();
        } else {
            return selectClause.append(whereClause).toString();
        }
    }
     ...
     * @return
     */
    private List<Serializable> parseParameter(List<Object> params) {
        List<Serializable> parsedParams = new ArrayList<>();
        for (Object object : params) {
            if (object instanceof EtatMessage) {
                ...
                // parse NiveauLecture
                String niveauLectureCode = NiveauLectureCodeAssembler.assembleXsdToNiveauLectureCode(
                    (NiveauLectureCode) object
                );
                ...
                if (EtatEvenement.EN_ATTENTE_DE_VALIDATION.toString().equals(string)) {
                    string = SolonEppLifecycleConstant.EVENEMENT_ATTENTE_VALIDATION_STATE;
                } else if (EtatEvenement.EN_INSTANCE.toString().equals(string)) {
                    string = SolonEppLifecycleConstant.EVENEMENT_INSTANCE_STATE;
                } else if (EtatEvenement.BROUILLON.toString().equals(string)) {
                    string = SolonEppLifecycleConstant.EVENEMENT_BROUILLON_STATE;
                } else if (EtatEvenement.ANNULE.toString().equals(string)) {
                    string = SolonEppLifecycleConstant.EVENEMENT_ANNULE_STATE;
                } else if (EtatEvenement.PUBLIE.toString().equals(string)) {
    ...
    private static class XSDQueryBuilder extends SDefaultQueryVisitor {
        ...
        public void visitDateLiteral(DateLiteral l) {
            if (isInWhere()) {
                ...
                throw new RuntimeException("Not supported operation : visitDateLiteral in " + pos);
            }
        }
        ...
        public void visitDoubleLiteral(DoubleLiteral l) {
            if (isInWhere()) {
                ...
                throw new RuntimeException("Not supported operation : visitDoubleLiteral in " + pos);
            }
        }
        ...
        public void visitExpression(Expression expr) {
            ...
            if (isInWhere()) {
                ...
                throw new RuntimeException("Not supported operation : visitExpression in " + pos);
            }
        ...
        public void visitHavingClause(HavingClause clause) {
            throw new RuntimeException("visitHavingClause not supported");
        }
        ...
        public void visitIntegerLiteral(IntegerLiteral l) {
            if (isInWhere()) {
                ...
                throw new RuntimeException("Not supported operation : visitIntegerLiteral in " + pos);
            }
        }
        ...
        public void visitLiteralList(LiteralList l) {
            ...
            if (isInWhere()) {
                ...
                throw new RuntimeException("No support for literal list in another part than where (" + pos + ")");
            }
        ...
        public void visitOperator(Operator op) {
            if (isInWhere()) {
                ...
                throw new RuntimeException("Not supported operation : visitOperator in " + pos);
            }
        }
            ...
            if (isInWhere()) {
                ...
                if (!wherePart.toString().endsWith("f.ecm:fulltext = ")) {
                    ...
                    if (availableParams == null) {
                        throw new RuntimeException("No parameter in entry");
                    }
                    if (nbParamsRequired >= availableParams.length) {
                        throw new RuntimeException(
                            "Not enough parameter in entry [available=" + availableParams.length + "]"
                        );
                    }
                ...
                throw new RuntimeException("param supported only in where part");
            }
        }
