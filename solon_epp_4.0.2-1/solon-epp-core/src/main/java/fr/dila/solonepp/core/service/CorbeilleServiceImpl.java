...
public class CorbeilleServiceImpl implements CorbeilleService {
    ...
    private static final String QUERY_CORBEILLE_HAS_MESSAGE =
        "SELECT /*+ FIRST_ROWS(1) */ a.id as id " +
    ...
    private static final String QUERY_EVT_INFOS =
        "SELECT e.ecm:uuid AS uuid, " +
        E_EVT_PREFIX +
    ...
    private static final String QUERY_VERSION_INFOS =
        "SELECT v.ecm:uuid AS uuid, " +
        V_VER_PREFIX +
        ...
        AS_SQL +
        VER_COMMENTAIRE_ALIAS +
        " FROM Version as v ";

    private static final String QUERY_DOSSIER =
        "SELECT " +
        DOS_ALIAS_PREFIX +
        STSchemaConstant.DUBLINCORE_SCHEMA_PREFIX +
        SEP +
        STSchemaConstant.DUBLINCORE_TITLE_PROPERTY +
        AS_SQL +
        DOS_ID_DOSSIER_ALIAS +
        VIRG +
        D_DOS_PREFIX +
        SolonEppSchemaConstant.DOSSIER_DATE_DEPOT_TEXTE_PROPERTY +
        AS_SQL +
        DOS_DATE_DEP_TXT_ALIAS +
        VIRG +
        D_DOS_PREFIX +
        SolonEppSchemaConstant.DOSSIER_NUMERO_DEPOT_TEXTE_PROPERTY +
        AS_SQL +
        DOS_NUM_DEP_TXT_ALIAS +
        VIRG +
        D_DOS_PREFIX +
    ...
    @Override
    public void findMessage(CoreSession session, RechercherMessageContext rechercherMessageContext) {
        ...
        if (messageDocList != null) {
            ...
            List<RechercherMessageDTO> messageList = rechercherMessageContext
                .getRechercherMessageResponse()
                .getMessagetList();
    ...
    @Override
    public long findMessageCount(CoreSession session, String corbeilleId) {
    ...
    @Override
    public boolean hasCommunicationNonTraites(CoreSession session, String corbeilleId) {
        ...
        try {
            res =
                QueryUtils.doSqlQuery(
                    ...
                    QUERY_CORBEILLE_HAS_MESSAGE,
                    new Object[] { corbeilleId }
                );
     ...
     * @return
     */
    private List<DocumentModel> getMessagesList(
        CoreSession session,
        RechercherMessageContext rechercherMessageContext
    ) {
        ...
        if (messageCriteria == null && queryString == null && parametrizedQuery == null) {
            throw new NuxeoException("Mode de recherche de message non spécifié");
        }
                ...
                if (parametrizedQuery != null) {
                    messageDocList =
                        QueryUtils.doUFNXQLQueryAndFetchForDocuments(
                            ...
                            parametrizedQuery,
                            rechercherMessageRequest.getParamList().toArray(),
                            pageSize,
                            offset
                        );
                }
     ...
     */
    private void mapMessageWithDocs(
        DocumentModel messageDoc,
        List<RechercherMessageDTO> messageList,
        Map<String, RechercherMessageDTO> mapEvenementIdDto,
        Map<String, RechercherMessageDTO> mapVersionIdDto,
     ...
     * @param mapEvenementIdDTO
     */
    private void loadEvenementsForMessages(CoreSession session, Map<String, RechercherMessageDTO> mapEvenementIdDTO) {
        // chargement de l'événement
        if (!mapEvenementIdDTO.isEmpty()) {
            StringBuilder queryEvt = new StringBuilder(QUERY_EVT_INFOS)
                .append(STQueryConstant.WHERE)
                .append(QueryHelper.getQuestionMarkQueryWithColumn("e.ecm:uuid", mapEvenementIdDTO.keySet().size()));
     ...
     * @param mapVersionIdDto
     */
    private void loadVersionsForMessages(CoreSession session, Map<String, RechercherMessageDTO> mapVersionIdDto) {
        // Chargement de la version
        if (!mapVersionIdDto.isEmpty()) {
            StringBuilder queryVersion = new StringBuilder(QUERY_VERSION_INFOS)
                .append(STQueryConstant.WHERE)
                .append(QueryHelper.getQuestionMarkQueryWithColumn("v.ecm:uuid", mapVersionIdDto.keySet().size()));

            IterableQueryResult res = QueryUtils.doUFNXQLQuery(
                session,
                queryVersion.toString(),
                mapVersionIdDto.keySet().toArray()
            );
     ...
     * @param mapDossierIdDto
     */
    private void loadDossiersForMessages(CoreSession session, Map<String, List<RechercherMessageDTO>> mapDossierIdDto) {
            ...
            StringBuilder queryDossier = new StringBuilder(QUERY_DOSSIER)
                ...
                .append(
                    QueryHelper.getQuestionMarkQueryWithColumn(
                        ...
                        SEP +
                        STSchemaConstant.DUBLINCORE_TITLE_PROPERTY,
                        mapDossierIdDto.keySet().size()
                    )
                );

            IterableQueryResult res = QueryUtils.doUFNXQLQuery(
                session,
                queryDossier.toString(),
                mapDossierIdDto.keySet().toArray()
            );
