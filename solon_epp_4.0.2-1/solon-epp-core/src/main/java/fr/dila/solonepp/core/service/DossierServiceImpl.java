...
public class DossierServiceImpl implements DossierService {
    ...
    private static final String GET_DOSSIER_QUERY =
        "SELECT d.ecm:uuid as id FROM " + SolonEppConstant.DOSSIER_DOC_TYPE + " AS d " + " WHERE d.dos:idDossier = ?";

    @Override
    public void deleteDossier(final CoreSession session, final DocumentModel dossierDoc) {
        LOGGER.info(
            session,
            STLogEnumImpl.DEL_DOSSIER_TEC,
            "Suppression du dossier: " + dossierDoc.getTitle() + ", UUID: " + dossierDoc.getId()
        );
        session.removeDocument(dossierDoc.getRef());
    }

    @Override
    public DocumentModel getDossier(final CoreSession session, final String dossierId) {
        final Object[] param = new Object[] { dossierId };
        final List<DocumentModel> list = QueryUtils.doUFNXQLQueryAndFetchForDocuments(
            session,
            SolonEppConstant.DOSSIER_DOC_TYPE,
            GET_DOSSIER_QUERY,
            param,
            1,
            0
    ...
    @Override
    public List<DocumentModel> getDossierList(final CoreSession session, final Collection<String> ids) {
        ...
        query.append(StringHelper.getQuestionMark(ids.size()));
        query.append(") ");
    ...
    public DocumentModel getDossierRoot(final CoreSession session) {
        ...
        synchronized (this) {
            ...
            if (list == null || list.size() <= 0) {
                throw new NuxeoException("Racine des dossiers non trouvée");
            } else if (list.size() > 1) {
                throw new NuxeoException("Plusieurs racines des dossiers trouvées");
            }
    ...
    @Override
    public DocumentModel createDossier(final CoreSession session, final DocumentModel dossierDoc) {
        ...
        if (StringUtils.isEmpty(dossierTitle)) {
            throw new NuxeoException("L'identifiant du dossier est obligatoire");
        }
    ...
    @Override
    public DocumentModel createBareDossier(final CoreSession session) {
    ...
    @Override
    public void updateFicheDossier(final CoreSession session, final DocumentModel versionDoc) {
        ...
        final MetaDonneesDescriptor metadonneesDescriptor = metadonneesService.getEvenementType(
        ...
        for (final String name : evenementMetaDescriptor.getProperty().keySet()) {
            ...
            if (
                ...
                propertyDescriptor.isFicheDossier() &&
                !SolonEppSchemaConstant.EVENEMENT_DOSSIER_PROPERTY.equals(name)
            ) {
                try {
                    dossierDoc.setProperty(
        ...
        for (final String name : versionMetaDescriptor.getProperty().keySet()) {
            ...
            if (propertyDescriptor != null && propertyDescriptor.isFicheDossier()) {
                ...
                if (
                    name.equals(SolonEppSchemaConstant.DOSSIER_URL_DOSSIER_SENAT_PROPERTY) ||
    ...
    @Override
    public Map<String, List<PropertyDescriptor>> getFicheDossierFields(
        ...
        for (final String evtType : typeEvenementSet) {
            ...
            descriptorList.put(
                ...
                new ArrayList<PropertyDescriptor>(descriptorSet)
            );
    ...
    @Override
    public Set<String> getEvenementTypeDossierList(final CoreSession session, final DocumentModel dossierDoc) {
        final Set<String> typeEvenementSet = new HashSet<String>();
    ...
    @Override
    public boolean isIdDossierValid(String idDossier) {
        String regex = "^[0-9a-zA-Z_\\-]+$";
        return idDossier.matches(regex);
    }
}
