...
public class EvenementTypeServiceImpl extends DefaultComponent implements EvenementTypeService {
    ...
    public void registerContribution(
        ...
        if (extensionPoint.equals(EVENEMENT_TYPE_EXTENSION_POINT)) {
            ...
            List<EvenementTypeDescriptor> evenementByDescriptorList = evenementTypeByCategoryMap.get(
                descriptor.getProcedure()
            );
    ...
    @Override
    public EvenementTypeDescriptor getEvenementType(final String evenementType) {
    ...
    @Override
    public boolean isTypeCreateur(final String evenementType) {
        return getEvenementType(evenementType).isCreateur();
    }

    @Override
    public boolean isTypeSuccessif(final String evenementType) {
        return getEvenementType(evenementType).isSuccessif();
    }

    @Override
    public boolean isDemandeAr(final String evenementType) {
        return getEvenementType(evenementType).isDemandeAr();
    }

    @Override
    public boolean isEmetteurAutorise(final String evenementType, final String emetteur) {
        final DistributionDescriptor distribution = getEvenementType(evenementType).getDistribution();
        if (distribution == null) {
            throw new NuxeoException(
                "La distribution n'est pas renseigné pour le type de communication : " + evenementType
            );
        }
    ...
    @Override
    public boolean isDestinataireAutorise(final String evenementType, final String destinataire) {
        final DistributionDescriptor distribution = getEvenementType(evenementType).getDistribution();
        if (distribution == null) {
            throw new NuxeoException(
                "La distribution n'est pas renseigné pour le type de communication : " + evenementType
    ...
    @Override
    public boolean isDestinataireCopieAutorise(final String evenementType, final String destinataireCopie) {
        final DistributionDescriptor distribution = getEvenementType(evenementType).getDistribution();
        if (distribution == null) {
            throw new NuxeoException(
                "La distribution n'est pas renseigné pour le type de communication : " + evenementType
    ...
    @Override
    public boolean isDestinataireCopieObligatoire(final String evenementType) {
        final DistributionDescriptor distribution = getEvenementType(evenementType).getDistribution();
        if (distribution == null) {
            throw new NuxeoException(
                "La distribution n'est pas renseigné pour le type de communication : " + evenementType
    ...
    @Override
    public boolean isPieceJointeObligatoire(final String evenementType, final String pieceJointeType) {
        final Map<String, PieceJointeDescriptor> pieceJointeDescriptorMap = getEvenementType(evenementType)
            .getPieceJointe();
    ...
    @Override
    public boolean isCreerBrouillon(final String evenementType) {
        return getEvenementType(evenementType).isCreerBrouillon();
    }

    @Override
    public boolean isCompleter(final String evenementType) {
        return getEvenementType(evenementType).isCompleter();
    }

    @Override
    public boolean isRectifier(final String evenementType) {
        return getEvenementType(evenementType).isRectifier();
    }

    @Override
    public boolean isAnnuler(final String evenementType) {
    ...
    @Override
    public List<EvenementTypeDescriptor> findEvenementTypeSuccessif() {
    ...
    @Override
    public List<EvenementTypeDescriptor> findEvenementTypeSuccessifWithSameProcedure(
        final String currentEvenementType
    ) {
        final List<EvenementTypeDescriptor> evenementSuccessifList = new ArrayList<EvenementTypeDescriptor>();
        ...
        for (final String evtType : evenementTypeMap.keySet()) {
            final EvenementTypeDescriptor evenementTypeDescriptor = evenementTypeMap.get(evtType);
            if (
                evenementTypeDescriptor.isSuccessif() &&
                (
                    currentEvtTypeDescriptor
                        .getProcedure()
                        .equals(SolonEppVocabularyConstant.CATEGORIE_EVENEMENT_DIVERS_VALUE) ||
                    evenementTypeDescriptor.getProcedure().equals(currentEvtTypeDescriptor.getProcedure()) ||
                    evenementTypeDescriptor
                        .getProcedure()
                        .equals(SolonEppVocabularyConstant.CATEGORIE_EVENEMENT_DIVERS_VALUE)
                )
            ) {
    ...
    @Override
    public List<EvenementTypeDescriptor> findEvenementTypeSuccessif(final String evenementType) {
        ...
        for (final String evtType : evenementTypeMap.keySet()) {
            final EvenementTypeDescriptor evenementTypeDescriptor = evenementTypeMap.get(evtType);
            if (
                evenementTypeDescriptor.isSuccessif() && evtSuccessifIdList.contains(evenementTypeDescriptor.getName())
            ) {
                evenementSuccessifList.add(evenementTypeDescriptor);
            }
        }
    ...
    public List<EvenementTypeDescriptor> findEvenementTypeSuccessif(
        ...
        for (final String evtType : evenementTypeMap.keySet()) {
            ...
            if (
                evenementTypeDescriptor.isSuccessif() &&
                evtSuccessifIdList.contains(evenementTypeDescriptor.getName()) &&
                evenementTypeDescriptor
                    .getDistribution()
                    .getEmetteur()
                    .getInstitution()
