...
public class MailboxInstitutionServiceImpl extends DefaultComponent implements MailboxInstitutionService {
    ...
    @Override
    public DocumentModel getMailboxInstitution(CoreSession session, String institutionId) {
        ...
        if (mailbox == null) {
            LOGGER.debug(
                session,
                STLogEnumImpl.FAIL_GET_INSTITUTION_MAIL_BOX,
                "Pas de mailbox avec l'id=<" + mailboxId + ">"
            );
            return null;
        }
     ...
     */
    protected Mailbox createMailboxInstitution(
        CoreSession session,
        DocumentModel mailboxRoot,
    ...
    @Override
    public void createAllMailboxInstitution(CoreSession session, DocumentModel mailboxRoot) {
