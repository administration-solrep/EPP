...
public class MessageServiceImpl implements MessageService {
    ...
    private static final String FIND_MESSAGE_BY_EVENEMENT_UUID_QUERY =
    ...
    public DocumentModel createBareMessage(CoreSession session, DocumentModel evenementDoc, DocumentModel mailboxDoc) {
        DocumentModel messageDoc = session.createDocumentModel(
            mailboxDoc.getPathAsString(),
    ...
    @Override
    public void deleteMessageByEvenementId(CoreSession session, final String evenementId) {
        ...
        new UnrestrictedSessionRunner(session) {
            ...
            public void run() {
                ...
                DocumentRef[] refs = QueryUtils.doUFNXQLQueryForIds(
                    session,
                    FIND_MESSAGE_BY_EVENEMENT_UUID_QUERY,
                    params
                );
    ...
    @Override
    public DocumentModel getMessageByEvenementId(CoreSession session, String evenementId) {
        MessageCriteria messageCriteria = new MessageCriteria();
        messageCriteria.setEvenementId(evenementId);
        messageCriteria.setCheckReadPermission(true);

        MessageDao messageDao = new MessageDao(session, messageCriteria, new PageInfo(1L, 0L));
    ...
    @Override
    public List<DocumentModel> getMessagesByEvenementIdWithoutReadPerm(
    ...
    @Override
    public void followTransitionEnCours(CoreSession session, String evenementId) {
        ...
        if (messageDoc == null) {
            throw new NuxeoException("Message non trouvé");
        }

        // Vérifie le type du message
        Message message = messageDoc.getAdapter(Message.class);
        if (message.isTypeEmetteur()) {
            throw new NuxeoException("Il est interdit de notifier une transition sur le message de l'émetteur");
        }
    ...
    @Override
    public void followTransitionTraite(CoreSession session, String evenementId) {
        LOGGER.info(session, EppLogEnumImpl.NOTIFICATION_TRANSITION_COMMUNICTION_TEC);
    ...
    @Override
    public void majInterne(CoreSession session, MajInterneContext majVisaInterneContext) {
        ...
        if (evenementDoc == null) {
            throw new NuxeoException("La communication n'a pas été trouvée.");
        }
        ...
        if (messageDoc != null) {
            ...
            evenementDistributionService.updateMessageMajVisaInterne(
                session,
                dossierDoc,
                evenementDoc,
                versionDoc,
