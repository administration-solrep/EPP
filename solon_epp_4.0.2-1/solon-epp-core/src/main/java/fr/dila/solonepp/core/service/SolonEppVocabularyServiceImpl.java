...
public class SolonEppVocabularyServiceImpl extends VocabularyServiceImpl implements SolonEppVocabularyService {
    ...
    public List<VocabularyEntry> getRubriquesEntriesForEmetteurs(final Collection<InstitutionsEnum> emetteurs) {
        ...
        try {
            ...
            DocumentModelList docs = vocabularySession.query(
                getRubriquesFiltersFromEmmetteurs(emetteurs),
                filter,
                orderBy
            );
            for (DocumentModel doc : docs) {
                entries.add(
                    new VocabularyEntry(
                        doc.getId(),
                        PropertyUtil.getStringProperty(doc, RUBRIQUES_VOCABULARY, STVocabularyConstants.COLUMN_LABEL)
                    )
                );
            }
        } catch (DirectoryException dex) {
            LOGGER.error(STLogEnumImpl.FAIL_OPEN_DIRECTORY_TEC, dex);
        } catch (NuxeoException cex) {
    ...
    @Override
    public String getRubriqueIdForLabel(String label) {
        List<DocumentModel> rubriqueSuggestions = getListDocumentModelSuggestions(
            label,
