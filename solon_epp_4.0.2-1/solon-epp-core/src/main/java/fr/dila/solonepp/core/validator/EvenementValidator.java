...
public class EvenementValidator {
     ...
     */
    public void validateEvenementSuccessif(
        ...
        if (!evenementTypeService.isTypeSuccessif(typeEvenement)) {
            throw new NuxeoException("Dossier existant : la communication doit être de type successif");
        }
        ...
        if (evenementParentDoc == null) {
            throw new NuxeoException("Communication parent non trouvé: " + evenementParentId);
        }
        ...
        if (!evenementParent.getDossier().equals(dossierDoc.getTitle())) {
            throw new NuxeoException(
                "La communication parent doit appartenir au dossier: " + evenementParent.getDossier()
            );
        }
     ...
     */
    public void validateEvenementStandard(DocumentModel evenementDoc, DocumentModel evenementParentDoc) {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        final EvenementTypeService evenementTypeService = SolonEppServiceLocator.getEvenementTypeService();
        ...
        if (
            evenementTypeService.isEvenementTypeAlerte(evenementType) ||
            evenementTypeService.isEvenementTypeGenerique(evenementType)
        ) {
            return;
        }
        ...
        do {
            Evenement evenementParent = evenementParentDoc.getAdapter(Evenement.class);
            EvenementTypeDescriptor evtParentTypeDescriptor = evenementTypeService.getEvenementType(
                evenementParent.getTypeEvenement()
            );
            EvenementTypeDescriptor evtTypeDescriptor = evenementTypeService.getEvenementType(
            ...
            if (evenementTypeService.isEvenementTypeAlerte(evenementParent.getTypeEvenement())) {
                throw new NuxeoException(
                    ...
                    evenementType +
                    " ne peut pas succéder à une communication de type Alerte"
                );
            }
            ...
            if (
                !evtParentTypeDescriptor.getProcedure().equals(evtTypeDescriptor.getProcedure()) &&
                !evtTypeDescriptor.getProcedure().equals(SolonEppVocabularyConstant.CATEGORIE_EVENEMENT_DIVERS_VALUE)
            ) {
                throw new NuxeoException(
                    "L'évènement à valider n'a pas une procédure valide (procédure de la communication précédente ou générique)"
                );
            }
     ...
     */
    public void validateEvenementAlerte(DocumentModel evenementDoc, DocumentModel versionDoc) {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        final EvenementTypeService evenementTypeService = SolonEppServiceLocator.getEvenementTypeService();
        ...
        if (version.isPositionAlerte()) {
            ...
            if (StringUtils.isEmpty(brancheAlerte)) {
                throw new NuxeoException(
                    "Une communication alerte (levée) doit succéder à une communication alerte (pose)"
                );
            } else if (!SolonEppSchemaConstant.EVENEMENT_BRANCHE_ALERTE_POSEE_VALUE.equals(brancheAlerte)) {
                throw new NuxeoException(
                    "Une communication alerte (levée) ne peut pas être créé après une alerte déjà levée"
                );
            }
        }
    }
     ...
     */
    public void validateDistribution(DocumentModel evenementDoc, boolean publie) {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);

        // Vérifie l'émetteur
        final String emetteur = evenement.getEmetteur();
        if (StringUtils.isBlank(emetteur)) {
            throw new NuxeoException("Émetteur est obligatoire");
        }

        final EvenementTypeService evenementTypeService = SolonEppServiceLocator.getEvenementTypeService();
        String evenementType = evenement.getTypeEvenement();
        if (!evenementTypeService.isEmetteurAutorise(evenementType, emetteur)) {
            throw new NuxeoException("Vous n'êtes pas autorisé à émettre le type de communication " + evenementType);
        }
        ...
        if (publie) {
            ...
            if (StringUtils.isBlank(destinataire)) {
                throw new NuxeoException("Destinataire est obligatoire");
            }

            if (destinataire.equals(emetteur)) {
                throw new NuxeoException(
                    "Le destinataire " + destinataire + " doit être différent de l'émetteur " + emetteur
                );
            }
            ...
            if (!evenementTypeService.isDestinataireAutorise(evenementType, destinataire)) {
                throw new NuxeoException(
                    "Le destinataire " +
                    destinataire +
                    " n'est pas autorisé pour le type de communication " +
                    evenementType
                );
            }
            ...
            if (destinataireCopieList == null || destinataireCopieList.isEmpty()) {
                if (evenementTypeService.isDestinataireCopieObligatoire(evenementType)) {
                    throw new NuxeoException(
                        "Le destinataire en copie est obligatoire pour le type de communication " + evenementType
                    );
                }
            } else {
                for (String destinataireCopie : destinataireCopieList) {
                    if (destinataireCopie.equals(emetteur)) {
                        throw new NuxeoException(
                            ...
                            destinataireCopie +
                            " doit être différent de l'émetteur " +
                            emetteur
                        );
                    }
                    ...
                    if (destinataireCopie.equals(destinataire)) {
                        throw new NuxeoException(
                            "Le destinataire en copie " +
                            destinataireCopie +
                            " doit être différent du destinataire " +
                            destinataire
                        );
                    }
     ...
     * @param evenementDoc Document événement à valider
     */
    public void validateInstitutionEmettrice(DocumentModel evenementDoc) {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        EppPrincipal principal = (EppPrincipal) session.getPrincipal();
        if (
            !principal.getInstitutionIdSet().contains(evenement.getEmetteur()) &&
            !InstitutionsEnum.isInstitutionAlwaysAccessible(evenement.getEmetteur())
        ) {
     ...
     * @param evenementDoc Document événement à valider
     */
    public void validateInstitutionDestinataire(DocumentModel evenementDoc) {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        EppPrincipal principal = (EppPrincipal) session.getPrincipal();
        if (!principal.getInstitutionIdSet().contains(evenement.getDestinataire())) {
            throw new NuxeoException("Seule l'institution destinataire peut modifier cette communication");
        }
    }
     ...
     */
    public void validateEtatEvenement(DocumentModel evenementDoc, Set<String> etatSet) {
        if (!etatSet.contains(evenementDoc.getCurrentLifeCycleState())) {
            throw new NuxeoException("La communication doit être à l'état " + StringUtils.join(etatSet, " ou "));
        }
    }
     ...
     * @param evenementDoc Document événement à valider
     */
    public void validateCreerBrouillon(DocumentModel evenementDoc) {
        final EvenementTypeService evenementTypeService = SolonEppServiceLocator.getEvenementTypeService();
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        String evenementType = evenement.getTypeEvenement();
        if (!evenementTypeService.isCreerBrouillon(evenementType)) {
            throw new NuxeoException(
                "Vous n'êtes pas autorisé à créer une version brouillon pour le type de communication " + evenementType
            );
        }
    }
     ...
     * @param evenementDoc Document événement à valider
     */
    public void validateCompleter(DocumentModel evenementDoc) {
        final EvenementTypeService evenementTypeService = SolonEppServiceLocator.getEvenementTypeService();
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        String evenementType = evenement.getTypeEvenement();
        if (!evenementTypeService.isCompleter(evenementType)) {
            throw new NuxeoException(
                "Vous n'êtes pas autorisé à compléter pour le type de communication " + evenementType
            );
        }
    }
     ...
     * @param evenementDoc Document événement à valider
     */
    public void validateRectifier(DocumentModel evenementDoc) {
        final EvenementTypeService evenementTypeService = SolonEppServiceLocator.getEvenementTypeService();
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        String evenementType = evenement.getTypeEvenement();
        if (!evenementTypeService.isRectifier(evenementType)) {
            throw new NuxeoException("Vous n'êtes pas autorisé à rectifier le type de communication " + evenementType);
        }
    }
     ...
     * @param evenementDoc Document événement à valider
     */
    public void validateAnnuler(DocumentModel evenementDoc) {
        final EvenementTypeService evenementTypeService = SolonEppServiceLocator.getEvenementTypeService();
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        String evenementType = evenement.getTypeEvenement();
        if (!evenementTypeService.isAnnuler(evenementType)) {
            throw new NuxeoException("Vous n'êtes pas autorisé à annuler le type de communication " + evenementType);
        }
    }
    ...
    public void validateMetaObligatoire(DocumentModel evenementDoc, boolean publie) {
        Evenement evenement = evenementDoc.getAdapter(Evenement.class);
        ...
        for (PropertyDescriptor propertyDescriptor : evtMetaDonneesDescriptor.getProperty().values()) {
            ...
            if (
                !publie &&
                (
                    SolonEppSchemaConstant.EVENEMENT_DESTINATAIRE_PROPERTY.equals(property) ||
            ...
            if (propertyDescriptor.isObligatoire()) {
                ...
                if (value == null) {
                    throw new NuxeoException("La propriété " + schema + ":" + property + " est obligatoire");
